...
public class dext5handler : IHttpHandler, IRequiresSessionState 
{
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        //parameterVo.IsDebug = true;
        //parameterVo.LogType = "C";
        //parameterVo.LogLevel = "DEBUG";
        //parameterVo.DebugFilePath = @"D:\log\";
        ...
        //Raonwiz.Dext5.DEXT5ParameterVo parameterVo = new Raonwiz.Dext5.DEXT5ParameterVo();
        //parameterVo.IsDebug = true;
        //parameterVo.LogType = "L";
        //parameterVo.LogConfigPath = System.IO.Path.Combine(strRootFolder, "handler" + cPathChar + ".NET" + cPathChar);
        //upload.SetDebugMode(parameterVo);
        ...
        upload.SetPhysicalPath = System.IO.Path.Combine(strRootFolder, "dext5uploaddata");
        ...
        string[] allowDownloadDirectoryPath = {System.IO.Path.Combine(strRootFolder, "dext5uploaddata"), context.Request.MapPath("/")};
        upload.SetAllowDownloadDirectoryPath = allowDownloadDirectoryPath;
        */
        //-------------------- [설정방법1] 물리적 경로 설정 끝 --------------------//

        //-------------------- [설정방법2] 가상경로 설정 시작 --------------------//
        string sVirtualPath = "/" + ZumNet.Framework.Configuration.Config.Read("UploadPath"); //"/dext5data/sjaekim";
        upload.SetVirtualPath = sVirtualPath + "/" + context.Session["LogonID"].ToString(); // "/dext5uploaddata";

        // 임시파일 물리적 경로설정 ( SetVirtualPath에 설정된 경로 + dext5temp )
        upload.SetTempPath = context.Request.MapPath(sVirtualPath) + cPathChar + "dext5temp";

        // ***************보안 설정 : 업로드 가능한 경로 설정 - 이외의 경로로 업로드 불가능***************
        string[] allowUploadDirectoryPath = { context.Request.MapPath(sVirtualPath) };
        upload.SetAllowUploadDirectoryPath = allowUploadDirectoryPath;
        ...
        upload.SetUploadCheckFileExtension(0, "exe,bat,sh,asp,aspx,ashx,inc,dll,asa,msi,vbs");
    ...
    private void Dext5_OpenDownloadPathChangeEventEx(Raonwiz.Dext5.Process.Entity.UploadEventEntity parameterEntity)
    {
    ...
    private void Dext5_LoggerEvent(Raonwiz.Dext5.Process.Entity.UploadEventEntity parameterEntity)
    {
        //로그 출력 전에 발생하는 이벤트 입니다.
        //로그 출력 전에 로그 정보를 변경하거나 로그 출력 여부를 변경하려는 경우 사용합니다.

        //string strDateTime = parameterEntity.Datetime; //DateTime Value
        //string strPathInfo = parameterEntity.PathInfo; //PathInfo Value
        //string strLogMode = parameterEntity.LogMode; //LogMode Value (FATAL,ERROR,WARN,INFO,DEBUG)
        //string strLogText = parameterEntity.LogText; //LogText Value

        //parameterEntity.Datetime = strDateTime; //Change DateTime Value
        //parameterEntity.PathInfo = strPathInfo; //Change PathInfo Value
        //parameterEntity.LogMode = strLogMode; //Change LogMode Value (FATAL,ERROR,WARN,INFO,DEBUG)
