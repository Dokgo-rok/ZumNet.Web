...
d.encryptFile,w)):n()}function makeGuidTagName(b){var g=0,f=(new Date).getTime().toString(32),e;if("undefined"!=typeof crypto&&"undefined"!=typeof crypto.randomUUID)f+=crypto.randomUUID().replace(/-/g,"").substring(0,20);else for(e=0;5>e;e++)f+=Math.floor(65535*Math.random()).toString(32);return(b||"o_")+f+(g++).toString(32)}
...
c.length&&(-1<c[2].indexOf("\b")?self.postMessage({type:"complete",id:b,originalFileName:e,uploadFilePath:d,uploadFileName:c[1],uploadCustomValue:c[2].split("\b")[0],uploadGroupId:c[2].split("\b")[1]}):self.postMessage({type:"complete",id:b,originalFileName:e,uploadFilePath:d,uploadFileName:c[1],uploadCustomValue:c[2],uploadGroupId:""}));XHRFactory.release(l)}}else self.postMessage({type:"error",code:"200004",message:"Http Status Code "+l.status,id:b,xhr:{status:l.status,responseText:l.responseText}}),
...
(upload_merge(m,g,h,e,f,d,k,q,t,r,l),numberOfServerUploadedChunks=0),XHRFactory.release(p))}else self.postMessage({type:"error",code:"200004",message:"Http Status Code "+p.status,id:m,xhr:{status:p.status,responseText:p.responseText}}),XHRFactory.release(p),self.close()};var n="",w=function(){try{p.send(n)}catch(a){self.postMessage({type:"reupload",chunk:b,filename:g,chunkInfo:e,asyncstate:d,guid:h,configValues:k,id:m,mark:q,gpid:t,fidx:r}),XHRFactory.release(p)}n=b=null};if("undefined"==typeof FormData){var a=
