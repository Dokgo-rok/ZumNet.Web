            ...
            if (_zw.V.ft == 'DRAFT' || _zw.V.ft == 'LEEVINDRAFT') { //크레신 - 일반기안
                ...
                if (_zw.V.biz == "normal" && _zw.V.act == '_reviewer') {
                    ...
                    $('#__subtable1 tr.sub_table_row .ft-sub-sub').each(function () {
                        if ($(this).find('[name="WHOMONEY"]').val() == '당사') {
                            s += parseFloat(_zw.ut.empty($(this).find('[name="CONVPRODUCTCOSTSUM"]').val()));
                            bWho = true; //당사가 있는 경우
                        }
                    });
                    ...
                    if (bWho) {
                        if (s == 0) {
                            if (p.length > 0) { bootbox.alert("당사 금형비 합계가 0인 경우 최종승인권자를 지정 할 수 없습니다!"); return 'N'; }
                        } else if (s < 5000) {
