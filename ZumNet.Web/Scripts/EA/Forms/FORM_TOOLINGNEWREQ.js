    ...
    _zw.formEx = {
		"validation": function (cmd) {
			...
			if (cmd == "draft") { //기안
				...
				if (_zw.V.biz == 'application') {
					el = $('#__mainfield[name="NEXTWORKER"]');
					if (el.length > 0 && $.trim(el.val()) == '') { bootbox.alert("필수항목 [기안자지정] 누락!", function () { el.focus(); }); return false; }
				}
			}
            return true;
        },
		"make": function (f) {
			if ((_zw.V.biz == 'receive' && _zw.V.act == "__r") || _zw.V.biz == 'application') _zw.body.main(f, ["NEXTWORKER", "NEXTWORKERID", "NEXTWORKERDEPT", "NEXTWORKERDEPTID"]);
        },
		...
		"date": function (el) {
			...
			if (from && from.val() != '' && to && to.val() != '') {
				...
				$('#__mainfield[name="DAYS"]').val(dif + 1);
			}
		},
			...
			$.ajax({
				...
				success: function (res) {
					...
					if (res.substr(0, 2) == 'OK') {
						...
						var row = x == 'TOOLINGTYPE' ? el.parent().parent() : null;
				...
				$.ajax({
					...
					success: function (res) {
						...
						if (res.substr(0, 2) == 'OK') {
							...
							var row = x == 'ITEMNO' ? el.parent().parent() : null;
