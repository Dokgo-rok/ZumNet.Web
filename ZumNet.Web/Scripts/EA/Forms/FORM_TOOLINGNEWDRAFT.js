			...
			if (cmd == "draft") { //기안
				...
				if (_zw.V.biz == '영업수신' && _zw.V.act == '__r') {
					...
					$('#__subtable1 tr.sub_table_row .ft-sub-sub').each(function () {
						...
						else if (e1.val() == '고객' && e7.val() == '') { bootbox.alert("필수항목 [비용회수사업장] 누락!", function () { }); bReturn = false; return false; }
						else if (e1.val() == '고객' && e8.val() == '') { bootbox.alert("필수항목 [비용회수예정일] 누락!", function () { }); bReturn = false; return false; }
					});
				}
			}
			return bReturn;
