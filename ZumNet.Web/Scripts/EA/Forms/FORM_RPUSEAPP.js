    ...
    _zw.formEx = {
        "validation": function (cmd) {
            ...
            if (cmd == "draft") { //기안
                ...
                if (_zw.V.biz == 'manage' && _zw.V.act == "_approver") {
                    $('#__subtable1 tr.sub_table_row').each(function () {
                        //console.log($(this).find('td:has([name="ROWSEQ"])'));
                        if ($(this).find('td:has([name="ROWSEQ"])').length > 0) {
                            el = $(this).find('input[name="USESTDT"]');
                            if (el.length > 0 && $.trim(el.val()) == '') { bootbox.alert("필수항목 [사용일자] 누락!", function () { el.focus(); }); bReturn = false; return false; }

                            el = $(this).find('input[name="USEEDDT"]');
                            if (el.length > 0 && $.trim(el.val()) == '') { bootbox.alert("필수항목 [사용일자] 누락!", function () { el.focus(); }); bReturn = false; return false; }

                            el = $(this).find('input[name="USESTTM"]');
                            if (el.length > 0 && $.trim(el.val()) == '') { bootbox.alert("필수항목 [사용시간] 누락!", function () { el.focus(); }); bReturn = false; return false; }

                            el = $(this).find('input[name="USEEDTM"]');
                            if (el.length > 0 && $.trim(el.val()) == '') { bootbox.alert("필수항목 [사용시간] 누락!", function () { el.focus(); }); bReturn = false; return false; }
                        }
                    });
                }
        ...
        "make": function (f) {
            if (_zw.V.biz == 'application' && _zw.V.act == "_approver") _zw.body.subPart(f, "1;EQUIPID;EQUIPNM");
            else if (_zw.V.biz == 'manage' && _zw.V.act == "_approver") _zw.body.subPart(f, "1;USESTDT;USEEDDT;USESTTM;USEEDTM;MTRUSE1;MTRUSE2;MTRUSE3;SUPPORT");
        ...
        "date": function (el) {
            var from, to, row;
            if (el.name == 'USESTDT' || el.name == 'USEEDDT') {
                row = $(el).parent().parent();
                from = row.find('input[name="USESTDT"]'); to = row.find('input[name="USEEDDT"]');
            }
            if (from && to) {
        ...
        "orgSelect": function (p, x) {
            var row = $(x).parent().parent(); //console.log(x)
            p.find('.zf-org .zf-org-select input:checkbox[data-for]').each(function () {
                ...
                row.find('input[name="APPLDN"]').val(dn);
                row.find('input[name="APPLID"]').val(info["id"]);
                row.find('input[name="APPLDEPT"]').val(info["grdn"]);
                row.find('input[name="APPLDEPTID"]').val(info["grid"]);
                row.find('input[name="APPLCORP"]').val(info["belong"]);
            });
            p.modal('hide');
        },
