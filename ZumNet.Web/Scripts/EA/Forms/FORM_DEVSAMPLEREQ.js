    ...
    _zw.formEx = {
        "validation": function (cmd) {
            ...
            if (cmd == "draft") { //기안
                ...
                if (_zw.V.biz == "application" && _zw.V.act == "__r") {
                    el = $('#__mainfield[name="DELIVERWAY"]');
                    if (el.length > 0 && $.trim(el.val()) == '') { bootbox.alert("필수항목 [입고방법] 누락!", function () { el.focus(); }); return false; }

                    el = $('#__mainfield[name="BLNO"]');
                    if (el.length > 0 && $.trim(el.val()) == '') { bootbox.alert("필수항목 [B/L No.] 누락!", function () { el.focus(); }); return false; }
                }
            }
            return true;
        },
        "make": function (f) {
            if (_zw.V.biz == "application" && _zw.V.act == "__r") _zw.body.main(f, ["DELIVERWAY", "BLNO"]);
        },
