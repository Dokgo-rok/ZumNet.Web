$(function () {
    _zw.formEx = {
		...
		"calc": function (el) {
			...
			} else if (el.name == "COUNT" || el.name == "UNITCOST1" || el.name == "UNITCOST2") {
				var row = $(el).parent().parent();
				s = parseFloat(_zw.ut.empty(row.find('td :text[name="COUNT"]').val())) * parseFloat(_zw.ut.empty(row.find('td :text[name="UNITCOST1"]').val()));
