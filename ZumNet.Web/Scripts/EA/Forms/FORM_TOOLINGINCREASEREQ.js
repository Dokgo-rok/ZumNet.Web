$(function () {
    _zw.formEx = {
        "validation": function (cmd) {
			...
			if (cmd == "draft") { //기안
				...
				if (_zw.V.biz == 'application' && _zw.V.act == '_approver') {
					el = $('#__mainfield[name="NEXTWORKER"]');
					if (el.length > 0 && $.trim(el.val()) == '') { bootbox.alert("필수항목 [증작기안자] 누락!", function () { el.focus(); }); bReturn = false; return false; }

				} else if (_zw.V.biz == '영업수신' && _zw.V.act == '__r') {
					var e1, e2, e3, e4;
		...
		"make": function (f) {
			if (_zw.V.biz == 'application' && _zw.V.act == "_approver") _zw.body.main(f, ["NEXTWORKER", "NEXTWORKERID", "NEXTWORKERDEPT", "NEXTWORKERDEPTID"]);
			else if (_zw.V.biz == '영업수신' && _zw.V.act == "__r") if (_zw.V.biz == '영업수신' && _zw.V.act == "__r") _zw.body.subPart(f, "1;WHOMONEY;WHOMONEYDETAIL;POSTYPE")
        },
		...
		"orgSelect": function (p, x) {
			p.find('.zf-org .zf-org-select input:checkbox[data-for]').each(function () {
				...
				$('#NEXTWORKERINFO').val(info["grdn"] + '. ' + dn);
					...
					success: function (res) {
						...
						if (res.substr(0, 2) == 'OK') {
							...
							p.find('.zf-modal .z-lnk-navy[data-val]').click(function () {
								...
								if (x == 'BUYER') _zw.formEx.searchToolingStatus('BB', v[0], '', '');
