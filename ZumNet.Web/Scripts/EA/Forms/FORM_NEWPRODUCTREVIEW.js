$(function () {
    _zw.formEx = {
        ...
        "calc": function (el) {
            if (el.name == 'COUNT1' || el.name == 'COUNT2') return false;
            var prefix = el.name.charAt(0), surfix = el.name.charAt(1), s = 0, f = '0,0.[0000]';
            var e1, e2, e3, e4;

            for (var i = 1; i <= 6; i++) {
                s += numeral(_zw.ut.empty($('#__mainfield[name="' + prefix + i.toString() + '"]').val())).value();
            }
            $('#__mainfield[name="SUM' + prefix + '"]').val(numeral(s).format(f));

            if (prefix == 'A' || prefix == 'B') {
                e1 = $('#__mainfield[name="A' + surfix + '"]'); e2 = $('#__mainfield[name="B' + surfix + '"]');
                e3 = $('#__mainfield[name="C' + surfix + '"]'); e4 = $('#__mainfield[name="D' + surfix + '"]');
                s = _zw.ut.sub(4, e1.val(), e2.val()); e3.val(numeral(s).format(f)); s = 0;
                s = _zw.ut.rate(e2.val(), e1.val(), 2); e4.val(numeral(s).format('0,0.00')); s = 0;

                e1 = $('#__mainfield[name="SUMA"]'); e2 = $('#__mainfield[name="SUMB"]');
                e3 = $('#__mainfield[name="SUMC"]'); e4 = $('#__mainfield[name="SUMD"]');
                s = _zw.ut.sub(4, e1.val(), e2.val()); e3.val(numeral(s).format(f)); s = 0;
                s = _zw.ut.rate(e2.val(), e1.val(), 2); e4.val(numeral(s).format('0,0.00')); s = 0;

            } else if (prefix == 'E' || prefix == 'F') {
                e1 = $('#__mainfield[name="E' + surfix + '"]'); e2 = $('#__mainfield[name="F' + surfix + '"]');
                e3 = $('#__mainfield[name="G' + surfix + '"]'); e4 = $('#__mainfield[name="H' + surfix + '"]');
                s = _zw.ut.sub(4, e1.val(), e2.val()); e3.val(numeral(s).format(f)); s = 0;
                s = _zw.ut.rate(e2.val(), e1.val(), 2); e4.val(numeral(s).format('0,0.00')); s = 0;

                e1 = $('#__mainfield[name="SUME"]'); e2 = $('#__mainfield[name="SUMF"]');
                e3 = $('#__mainfield[name="SUMG"]'); e4 = $('#__mainfield[name="SUMH"]');
                s = _zw.ut.sub(4, e1.val(), e2.val()); e3.val(numeral(s).format(f)); s = 0;
                s = _zw.ut.rate(e2.val(), e1.val(), 2); e4.val(numeral(s).format('0,0.00')); s = 0;

            } else if (prefix == 'I' || prefix == 'J') {
                e1 = $('#__mainfield[name="I' + surfix + '"]'); e2 = $('#__mainfield[name="J' + surfix + '"]');
                e3 = $('#__mainfield[name="K' + surfix + '"]'); e4 = $('#__mainfield[name="L' + surfix + '"]');
                s = _zw.ut.sub(4, e1.val(), e2.val()); e3.val(numeral(s).format(f)); s = 0;
                s = _zw.ut.rate(e2.val(), e1.val(), 2); e4.val(numeral(s).format('0,0.00')); s = 0;

                e1 = $('#__mainfield[name="SUMI"]'); e2 = $('#__mainfield[name="SUMJ"]');
                e3 = $('#__mainfield[name="SUMK"]'); e4 = $('#__mainfield[name="SUML"]');
                s = _zw.ut.sub(4, e1.val(), e2.val()); e3.val(numeral(s).format(f)); s = 0;
                s = _zw.ut.rate(e2.val(), e1.val(), 2); e4.val(numeral(s).format('0,0.00')); s = 0;
            }
        },
        ...
        "orgSelect": function (p, x) {
            p.find('.zf-org .zf-org-select input:checkbox[data-for]').each(function () {
                ...
                $('#__mainfield[name="MANAGER"]').val(dn);
                $('#__mainfield[name="DEPT"]').val(info["grdn"]);
            });
            p.modal('hide');
        },
