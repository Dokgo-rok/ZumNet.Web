$(function () {
    _zw.formEx = {
        "validation": function (cmd) {
            ...
            if (cmd == "draft") { //기안
                ...
                if (_zw.V.biz == "의견") {
                    ...
                    el = $('#__mainfield[name="CURRENCY2"]');
                    if (el.length > 0 && $.trim(el.val()) == '') { bootbox.alert("필수항목 [통화] 누락!", function () { el.focus(); }); return false; }

                    el = $('#__mainfield[name="TOTALSUM"]');
                    if (el.length > 0 && $.trim(el.val()) == '') { bootbox.alert("필수항목 [당사투자금액합] 누락!", function () { el.focus(); }); return false; }

                    el = $('#__mainfield[name="DEVPRICEBA"]');
                    if (el.length > 0 && $.trim(el.val()) == '') { bootbox.alert("필수항목 [내작가공비] 누락!", function () { el.focus(); }); return false; }

                    el = $('#__mainfield[name="DEVPRICEBB"]');
                    if (el.length > 0 && $.trim(el.val()) == '') { bootbox.alert("필수항목 [외주가공비] 누락!", function () { el.focus(); }); return false; }

                    el = $('#__mainfield[name="DEVPRICEA"]');
                    if (el.length > 0 && $.trim(el.val()) == '') { bootbox.alert("필수항목 [재료비] 누락!", function () { el.focus(); }); return false; }

                    el = $('#__mainfield[name="DEVPRICEC"]');
                    if (el.length > 0 && $.trim(el.val()) == '') { bootbox.alert("필수항목 [관리비] 누락!", function () { el.focus(); }); return false; }

                    el = $('#__mainfield[name="DEVELOPPRICE"]');
                    if (el.length > 0 && $.trim(el.val()) == '') { bootbox.alert("필수항목 [개발견적가] 누락!", function () { el.focus(); }); return false; }

                } else if (_zw.V.biz == "yeongeob" && _zw.V.act == "_approver") {
                    el = $('#__mainfield[name="TERM"]');
                    if (el.length > 0 && $.trim(el.val()) == '') { bootbox.alert("필수항목 [기간] 누락!", function () { el.focus(); }); return false; }

                    el = $('#__mainfield[name="CURRENCY1"]');
                    if (el.length > 0 && $.trim(el.val()) == '') { bootbox.alert("필수항목 [통화] 누락!", function () { el.focus(); }); return false; }

                    el = $('#__mainfield[name="SALEPRICE"]');
                    if (el.length > 0 && $.trim(el.val()) == '') { bootbox.alert("필수항목 [판매목표가] 누락!", function () { el.focus(); }); return false; }

                    el = $('#__mainfield[name="MCRATE"]');
                    if (el.length > 0 && $.trim(el.val()) == '') { bootbox.alert("필수항목 [MC율] 누락!", function () { el.focus(); }); return false; }

                    el = $('#__mainfield[name="ESTIMATESTD"]');
                    if (el.length > 0 && $.trim(el.val()) == '') { bootbox.alert("필수항목 [견적기준] 누락!", function () { el.focus(); }); return false; }

                    el = $('#__mainfield[name="PLANCOUNT"]');
                    if (el.length > 0 && $.trim(el.val()) == '') { bootbox.alert("필수항목 [기획수량] 누락!", function () { el.focus(); }); return false; }
                }
            }
            return true;
        },
        "make": function (f) {
            ...
            else if (_zw.V.biz == "yeongeob" && _zw.V.act == "_approver") _zw.body.main(f, ["CURRENCY1", "SALEPRICE", "MCRATE", "ESTIMATESTD", "PLANCOUNT", "PREDICTSALES", "PREDICTPERPROFIT", "PREDICTPROFIT", "PREDICTRATE", "PREDICTINVEST", "PREDICTWAY"]);
