$(function () {
    _zw.formEx = {
		...
		"calc": function (el) {
			...
			if (el.name == 'PREPRODUCTCOSTCURRENCY' || el.name == 'PREPRODUCTCOST') {
				var c = $(el).parent().parent().parent(); //tbody
				e1 = c.find('[name="PREPRODUCTCOSTCURRENCY"]'); e2 = c.find('[name="PREPRODUCTCOST"]');
				...
				success: function (res) {
					...
					if (res.substr(0, 2) == 'OK') {
						if (vPos[1] == 'SEARCH_TOOLING') {
							...
							var row = x == 'RTVLORGNM' || x == 'PRODUCTCOSTCURRENCY' || x == 'PREPRODUCTCOSTCURRENCY' ? el.parent().parent() : null;
							pop.find('a[data-val]').click(function () {
								...
								if (x == 'PRODUCTCOSTCURRENCY' || x == 'PREPRODUCTCOSTCURRENCY') _zw.formEx.calc(row.find('[name="' + x + '"]')[0]);
