...
<xsl:stylesheet  version="1.0"
  ...
  function linkCEMain(w, r, pn, pid) {if (pid.indexOf('_') > 0) {return pn;} else {return "<a href=\"javascript:\" onclick=\"var url = '/" + r + "/ExtensionService/CE/Grid.aspx?M=simul&app=" + pid + "&Q=lnk'; var w = window.screen.width; var h = window.screen.height; w = w >= 1400 ? parseInt(w * 0.85) : w; h = h >= 900 ? parseInt(h * 0.85) : h; var sy = window.screen.height / 2 - h / 2 - 70; var sx = window.screen.width  / 2 - w / 2; if (sy < 0) {sy = 0;} var param = 'toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=1,resizable=1,top=' + sy + ',left=' + sx + ',width=' + w + ',height=' + h; window.open(url,'ce_grid', param);\">" + pn + "</a>";}}
