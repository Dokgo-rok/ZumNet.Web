...
<xsl:stylesheet  version="1.0"
  ...
  function fieldValue4(p, comp, nm) {//2012-02-28
    ...
    if (rt == '') rt = isEmpty(nm);
      ...
      rt = "<a style=\"cursor:hand;color:#05a;\" onclick=\"var b=document.body,p=nextSibling;var st=b.scrollTop,oh=b.offsetHeight,h=event.y-40+st; with(p.style){position='absolute';left=event.x+80;top=h;display='block';}\">" + nm + "</a>" + rt;
    }
    return rt;
  }
  ...
  function setTick(t,l,w,h,c,sc,dt,tt) {
    ...
    t -= h/2; l -= w/2;
    return "<v:oval style=\"z-index:10;position:absolute;top:" + t + "px;left:" + l + "px;width:" + w + "px;height:" + h + "px;cursor:hand\" fillcolor=\"" + c + "\" strokecolor=\"" + sc + "\" title=\"" + tt + "\"><v:stroke dashstyle=\"" + dt + "\" linestyle=\"" + ls + "\" weight=\"" + sw + "\" />" + sText + "</v:oval>";
  }
