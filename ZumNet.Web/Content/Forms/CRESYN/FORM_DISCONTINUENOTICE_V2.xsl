            ...
            <table border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td style="width:245px">
                  <xsl:value-of disable-output-escaping="yes" select="phxsl:mappingSignRcvPart($root, //processinfo/signline/lines, 'distribution', '__si_Distribution', '3', '접수')"/>
                </td>
              </tr>
            </table>
          </div>
                ...
                <td colspan="2">
                  <table id="__subtable1" class="ft-sub" header="1"  border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr style="height:36px">
                      ...
                      <td class="f-lbl-sub" style="border-top:0">재고처리여부</td>
                      <!--<td class="f-lbl-sub" style="border-top:0">법인손실여부</td>-->
    ...
    <tr class="sub_table_row">
      ...
      <td>
        <xsl:choose>
          ...
          <xsl:when test="$bizrole='distribution' and $actrole='__r'">
            <input type="text" name="CLIENT">
          ...
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(CLIENT))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td>
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="MODELNAME" style="width:81%;margin-right:2px">
              <xsl:attribute name="class">txtText</xsl:attribute>              
              <xsl:attribute name="value">
                <xsl:value-of select="MODELNAME" />
              </xsl:attribute>
            </input>
            <!--<button onclick="parent.fnExternal('erp.items',240,40,80,0,'',this,'ITEMNAME');" onfocus="this.blur()" class="btn_bg" style="height:16px;">
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="PRODUCTCENTER" style="width:68%">
            ...
            <!--<button onclick="parent.fnOption('external.centercode',240,120,100,0,'',this);" onfocus="this.blur()" class="btn_bg" style="height:16px;">
              <img alt="" class="blt01" style="margin:0 0 2px 0">
                <xsl:attribute name="src">
                  /<xsl:value-of select="$root"/>/EA/Images/ico_28.gif
                </xsl:attribute>
          ...
          <xsl:when test="$bizrole='distribution' and $actrole='__r'">
            <input type="text" name="PRODUCTCENTER">
              <xsl:attribute name="class">txtRead</xsl:attribute>
              <xsl:attribute name="readonly">readonly</xsl:attribute>
              <xsl:attribute name="value">
                <xsl:value-of select="PRODUCTCENTER" />
          ...
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(PRODUCTCENTER))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td style="text-align:center">
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="DISCONTINUETIME" class="datepicker txtDate" maxlength="10" data-inputmask="date;yyyy-MM-dd" value="{DISCONTINUETIME}" />
          </xsl:when>
          <xsl:when test="$bizrole='distribution' and $actrole='__r'">
            <input type="text" name="DISCONTINUETIME">
              <xsl:attribute name="class">txtRead</xsl:attribute>
              <xsl:attribute name="readonly">readonly</xsl:attribute>
              <xsl:attribute name="value">
                <xsl:value-of select="DISCONTINUETIME" />
              </xsl:attribute>
            </input>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(DISCONTINUETIME))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <select name="REGICHECK" class="form-control">
              <xsl:choose>
                <xsl:when test="phxsl:isEqual(string(REGICHECK),'')">
              ...
              <xsl:choose>
                <xsl:when test="phxsl:isEqual(string(REGICHECK),'고객보상')">
                  <option value="고객보상" selected="selected">고객보상</option>
                </xsl:when>
                <xsl:otherwise>
                  <option value="고객보상">고객보상</option>
                </xsl:otherwise>
              </xsl:choose>
              <xsl:choose>
                <xsl:when test="phxsl:isEqual(string(REGICHECK),'타모델공용')">
                  <option value="타모델공용" selected="selected">타모델공용</option>
                </xsl:when>
                <xsl:otherwise>
                  <option value="타모델공용">타모델공용</option>
                </xsl:otherwise>
              </xsl:choose>
              <xsl:choose>
                <xsl:when test="phxsl:isEqual(string(REGICHECK),'재고없음')">
                  <option value="재고없음" selected="selected">재고없음</option>
                </xsl:when>
                <xsl:otherwise>
              ...
              <xsl:choose>
                <xsl:when test="phxsl:isEqual(string(REGICHECK),'기타처리')">
                  <option value="기타처리" selected="selected">기타처리</option>
                </xsl:when>
                <xsl:otherwise>
                  <option value="기타처리">기타처리</option>
                </xsl:otherwise>
              </xsl:choose>
            </select>
          </xsl:when>
          <xsl:when test="$bizrole='distribution' and $actrole='__r'">
            <input type="text" name="REGICHECK">
              <xsl:attribute name="class">txtRead</xsl:attribute>
              <xsl:attribute name="readonly">readonly</xsl:attribute>
              <xsl:attribute name="value">
                <xsl:value-of select="REGICHECK" />
              </xsl:attribute>
            </input>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(REGICHECK))" />
          </xsl:otherwise>
        </xsl:choose>

        <!--<xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="PRODUCTREGI" style="width:82%">
              <xsl:attribute name="class">txtText_u</xsl:attribute>
              <xsl:attribute name="readonly">readonly</xsl:attribute>
              <xsl:attribute name="value">
                <xsl:value-of select="PRODUCTREGI" />
              </xsl:attribute>
            </input>
            <button onclick="parent.fnOption('external.regicode',240,120,100,0,'',this);" onfocus="this.blur()" class="btn_bg" style="height:16px;">
              <img alt="" class="blt01" style="margin:0 0 2px 0">
                <xsl:attribute name="src">
                  /<xsl:value-of select="$root"/>/EA/Images/ico_28.gif
                </xsl:attribute>
          ...
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(PRODUCTREGI))" />
          </xsl:otherwise>
        </xsl:choose>-->
      </td>
      <!--<td style="text-align:center">
        <xsl:choose>
          <xsl:when test="$bizrole='distribution' and $actrole='__r'">
            <select name="LOSSCHECK" style="">
              <xsl:choose>
                <xsl:when test="phxsl:isEqual(string(LOSSCHECK),'')">
                  <option value="" selected="selected">선택</option>
                </xsl:when>
                <xsl:otherwise>
                  <option value="">선택</option>
              ...
              <xsl:choose>
                <xsl:when test="phxsl:isEqual(string(LOSSCHECK),'있음')">
              ...
              <xsl:choose>
                <xsl:when test="phxsl:isEqual(string(LOSSCHECK),'없음')">
          ...
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(LOSSCHECK))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>-->
      <td style="border-right:0">
