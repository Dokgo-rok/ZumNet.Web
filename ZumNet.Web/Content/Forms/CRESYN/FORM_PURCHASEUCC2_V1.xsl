            ...
            <table border="0" cellspacing="0" cellpadding="0">
              <xsl:choose>
                <xsl:when test="$mode='new' or $mode='edit'  or $actrole='__r' or $actrole='_reviewer'">
                  <tr>
                    <td>
                      <span style ="width:230px;color:red;font-weight:bold;">※ 환율변경시 통화를 재설정하세요 </span>
                    </td>
                    <td class="fm-button">
                      적용사업장 :
                      ...
                      <input type="text" id="__mainfield" name="EXCHANGERATE" style="width:100px;height:16px" class=" txtText_u txtDate" maxlength="8" onclick="parent.fnShowPopSelfCalendar(this, parent.fnCalcValid)" onfocusout="parent.fnDateFormat(this)"  readonly="readonly" value="{//forminfo/maintable/EXCHANGERATE}" />
                        <button onclick="parent.fnExchangeInfo2()" onfocus="this.blur()" class="btn_bg">
                          환율적용
                        </button>
                ...
                <td colspan="2">
                  <table id="__subtable1" class="ft-sub" header="1"  border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr style="height:40px">
                      ...
                      <td class="f-lbl-sub" style="border-top:0">개발<br />견적가</td>
                      ...
                      <td class="f-lbl-sub" style="border-top:0">업체<br />CODE</td>
                      <td class="f-lbl-sub" style="border-top:0">BPA<br />NUM</td>
                      <td class="f-lbl-sub" style="border-top:0">적용시점</td>                      
                      <td class="f-lbl-sub" style="border-top:0">TTL<br />L/T</td>
                      <td class="f-lbl-sub" style="border-top:0">Pre<br />L/T</td>
                      <td class="f-lbl-sub" style="border-top:0">L/T</td>
                      <td class="f-lbl-sub" style="border-top:0">Post<br />L/T</td>
                    ...
                    <tr>                      
                      <xsl:choose>
                        <xsl:when test="$mode='new' or $mode='edit'  or $actrole='__r' or $actrole='_reviewer'">
                          <td class="f-lbl-sub" colspan="13">TOTAL</td>
                        </xsl:when>
                        <xsl:otherwise>
                          <td class="f-lbl-sub" colspan="12">TOTAL</td>
                        </xsl:otherwise>
                      </xsl:choose>
  ...
  <xsl:template match="//forminfo/subtables/subtable1/row">
    <tr class="sub_table_row">
      <xsl:choose>
        <xsl:when test="$mode='new' or $mode='edit' or $actrole='__r' or $actrole='_reviewer'">
          <td>
                <input type="checkbox" name="ROWSEQ" value ="{ROWSEQ}"></input>
          </td>
        </xsl:when>
        <xsl:otherwise>          
        </xsl:otherwise>
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit' or $actrole='__r' or $actrole='_reviewer'">
            <input  name="ROWNUM" type="text" class ="txtNumberic"  maxlength="5" value="{ROWNUM}"> </input>
          </xsl:when>
          <xsl:otherwise>
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit' or $actrole='__r' or $actrole='_reviewer'">
            <input  name="MODELNAME" type="text" class ="txtText_u" readonly="readonly"  style="width:79%" value="{MODELNAME}"></input>
            <button onclick="parent.fnExternal('erp.items3',240,40,80,70,'',this);" onfocus="this.blur()" class="btn_bg" style="height:16px;">
              <img alt="" class="blt01" style="margin:0 0 2px 0" src="/{//config/@root}/EA/Images/ico_28.gif" />
            </button>
          </xsl:when>
          <xsl:otherwise>
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit' or $actrole='__r' or $actrole='_reviewer'">
            <input  name="ITEMNAME" type="text" class="txtRead" readonly="readonly"  value="{ITEMNAME}"> </input>
          </xsl:when>
          <xsl:otherwise>
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit' or $actrole='__r' or $actrole='_reviewer'">
            <input  name="ITEMNO" type="text"  class="txtText_u" readonly="readonly" style="width:79%" value="{ITEMNO}"></input>
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit' or $actrole='__r' or $actrole='_reviewer'">
            <input name="PRICEA" type="text" class="txtText" maxlength="20" value="{PRICEA}"></input>
          </xsl:when>
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Right</xsl:attribute>
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit' or $actrole='__r' or $actrole='_reviewer'">
            <input name="PRICEB" type="text" class="txtText" maxlength="20" value="{PRICEB}"></input>
          </xsl:when>
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Right</xsl:attribute>
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit' or $actrole='__r' or $actrole='_reviewer'">
            <input  name="PRICEC" type="text" class="txtText" maxlength="20" value="{PRICEC}" onkeyup="parent.exchagerateclac(this)"></input>
          </xsl:when>
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Right</xsl:attribute>
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit' or $actrole='__r' or $actrole='_reviewer'">
            <input  name="SUBCURRENCY" type="text"  style ="width:45px"  class="txtText" maxlength="20" value="{SUBCURRENCY}" ></input>
            <button onclick="parent.fnOption('iso.currency',160,140,10,115,'etc',this);" onfocus="this.blur()" class="btn_bg" style="height:16px;">
              <img alt="" class="blt01" style="margin:0 0 2px 0" src="/{//config/@root}/EA/Images/ico_28.gif" />
            </button>
            <input type="hidden" id="__mainfield" name="SUBEXCHANGERATE" value="{SUBEXCHANGERATE}"/>
          
          </xsl:when>
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Right</xsl:attribute>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(SUBCURRENCY))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit' or $actrole='__r' or $actrole='_reviewer'">
            <input name="PRICED" type="text" class="txtDollar5" style ="width:58px"  maxlength="20" value="{PRICED}"></input>
          </xsl:when>
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Right</xsl:attribute>
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit' or $actrole='__r' or $actrole='_reviewer'">
            <input name="COMPANY" type="text" class="txtText_u" readonly="readonly" style="width:83%" value="{COMPANY}"></input>
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit' or $actrole='__r' or $actrole='_reviewer'">
            <input name="COMPANYCODE" type="text" class="txtRead" readonly="readonly" value="{COMPANYCODE}"></input>
          </xsl:when>
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Center</xsl:attribute>
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'  or $actrole='__r' or $actrole='_reviewer'">
            <input name="BPANUM" type="text"  style="width:77%" class="txtText_u" readonly="readonly" value="{BPANUM}" />
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'  or $actrole='__r' or $actrole='_reviewer'">
            <input name="APPLYPOINT" type="text" class="txtDate" maxlength="8" onclick="parent.fnShowPopSelfCalendar(this, parent.fnCalcValid)" value="{APPLYPOINT}"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Center</xsl:attribute>
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit' or $actrole='__r' or $actrole='_reviewer'">
            <input name="TTLLT" type="text" class="txtCurrency" maxlength="20" value="{TTLLT}" />
          </xsl:when>
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Right</xsl:attribute>
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit' or $actrole='__r' or $actrole='_reviewer'">
            <input name="PRELT" type="text" class="txtCurrency" maxlength="20" value="{PRELT}" />
          </xsl:when>
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Right</xsl:attribute>
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit' or $actrole='__r' or $actrole='_reviewer'">
            <input name="PROCESSLT" type="text" class="txtCurrency" maxlength="20" value="{PROCESSLT}"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Right</xsl:attribute>
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit' or $actrole='__r' or $actrole='_reviewer'">
            <input  name="POSTLT" type="text" class="txtCurrency" maxlength="20" value="{POSTLT}"/>
          </xsl:when>
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Right</xsl:attribute>
      ...
      <td style="border-right:0">
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit' or $actrole='__r' or $actrole='_reviewer'">
            <input name="ETC" type="text" class="txtText" maxlength="50" value="{ETC}"/>
          </xsl:when>
          <xsl:otherwise>
