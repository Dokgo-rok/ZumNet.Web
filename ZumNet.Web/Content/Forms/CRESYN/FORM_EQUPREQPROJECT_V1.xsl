		...
		<html>
			<head>
				...
				<style type="text/css">
					...
					/* 폰트 */
					.fh h1 {font-family:굴림체;font-weight:bold;font-size:20.0pt;letter-spacing:1pt;}
			...
			<body>
				<div class="m">
					...
					<div class="fb">
						<table border="0" cellspacing="0" cellpadding="0">
							<tr>
								...
								<td style="width:27%;font-size:1px">&nbsp;</td>

								<td>
									<table border="0" cellspacing="0" cellpadding="0">
										<tr>
											<td>
												<xsl:value-of disable-output-escaping="yes" select="phxsl:mappingSignPart($root, //processinfo/signline/lines/line[@bizrole='application' and @partid!='' and @actrole='_manager' and @step!='0'], '__si_Application', '1')"/>
											</td>
										</tr>
									</table>
								</td>
                    ...
                    <!--프로젝트일때 필수 입력 시작-->
						<table class="ft" border="0" cellspacing="0" cellpadding="0">
							<tr>
                             <tr><td class="f-lbl" >신청구분</td>
								
                            	<td style="border-right:0" colspan="3">  <xsl:choose>
										<xsl:when test="$mode='read'">
											<input  style="width:100" type="text" name="DOCTITLE" id="__mainfield">
												<xsl:attribute name="class">txtRead</xsl:attribute>
												<xsl:attribute name="readonly">readonly</xsl:attribute>
												<xsl:attribute name="value">
													<xsl:value-of select="//forminfo/maintable/DOCTITLE" />
												</xsl:attribute>
											</input>
										</xsl:when>
										<xsl:otherwise>
											<select style="width:150" class="txtRead" name="DOCTITLE" id="__mainfield" onchange="parent.fnEquipEq(this.value)">
												<xsl:choose>
													<xsl:when test="//forminfo/maintable/DOCTITLE[.='']">
														
                                                        <option value="프로젝트신규" selected="true">프로젝트신규</option>
                                                        <option value="프로젝트변경">프로젝트변경</option>
													</xsl:when>
													
													<xsl:when test="//forminfo/maintable/DOCTITLE[.='프로젝트신규']">
													
                                                        <option value="프로젝트신규" selected="true">프로젝트신규</option>
                                                        <option value="프로젝트변경">프로젝트변경</option>			
													</xsl:when>
                                                    <xsl:when test="//forminfo/maintable/DOCTITLE[.='프로젝트변경']">
													
                                                        <option value="프로젝트신규">프로젝트신규</option>
                                                        <option value="프로젝트변경" selected="true">프로젝트변경</option>			
													</xsl:when>
													<xsl:otherwise>
								...
								<td class="f-lbl">프로젝트번호</td>
								<td style="width:35%">
									<input type="text" name="PROJNO" id="__mainfield">
										<xsl:choose>
											...
											<xsl:when test="$mode='edit'">
												...
												<xsl:attribute name="maxlength">50</xsl:attribute>
												<xsl:attribute name="value">
													<xsl:value-of select="//forminfo/maintable/PROJNO" />
												</xsl:attribute>
											</xsl:when>
											<xsl:otherwise>
												<xsl:attribute name="class">txtRead</xsl:attribute>
										...
										<xsl:if test="$mode!='read'">
											<xsl:attribute name="onkeyup">parent.PROJECTNO_onkeyup('project', 'PROJNAME', 'PROJNO');</xsl:attribute>
										</xsl:if>
									</input>
								</td>
								<td class="f-lbl">프로젝트명</td>
								<td style="border-right:0">
									<xsl:choose>
										<xsl:when test="$mode='new'">
											<input type="text" name="PROJNAME" id="__mainfield">
												<xsl:attribute name="class">txtText</xsl:attribute>
                                                <xsl:attribute name="style">border:1px solid red</xsl:attribute>
												<xsl:attribute name="maxlength">50</xsl:attribute>
											</input>
										...
										<xsl:when test="$mode='edit'">
											<input type="text" name="PROJNAME" id="__mainfield">
												...
												<xsl:attribute name="maxlength">50</xsl:attribute>
												<xsl:attribute name="value">
													<xsl:value-of select="//forminfo/maintable/PROJNAME" />
												</xsl:attribute>
											</input>
											</xsl:when>
										<xsl:otherwise>
											...
											<xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(forminfo/maintable/PROJNAME))" />
										</xsl:otherwise>
									</xsl:choose>
									<xsl:if test="$mode!='read'">
										<xsl:attribute name="onkeyup">parent.PROJECTNO_onkeyup('project', 'PROJNAME', 'PROJNO');</xsl:attribute>
                            ...
                            <tr>
                            	<td style="border-bottom:0;border-right:0" colspan="4"> <strong>* 프로젝트장비의 경우 실행품의서 반드시 첨부.</strong>
                                </td>
                            </tr>
						</table>
					</div>
					...
					<div class="fm">
						<table class="ft" border="0" cellspacing="0" cellpadding="0">
							<tr>
								...
								<td style="border-right:0;height:100px;border-bottom:0;vertical-align:top">
									<xsl:choose>
										<xsl:when test="$mode='read'">
						...
						<table border="0" cellspacing="0" cellpadding="0">
							<xsl:choose>
								<xsl:when test="$mode='new' or $mode='edit'">
									<tr>
                                    	<td align="left "><span>현재사용중인 장비</span></td>
							...
							<tr>
								<td colspan="2">
									<table id="__subtable1" class="ft-sub" header="1"  border="0" cellspacing="0" cellpadding="0">
										<tr style="height:25">
											<td class="f-lbl-sub" style="width:28px;border-top:0">No</td>
											<td class="f-lbl-sub" style="width:;border-top:0">현재 사용장비 목록</td>
											<td class="f-lbl-sub" style="width:50px;border-top:0">수량</td>
                                            <td class="f-lbl-sub" style="width:150px;border-top:0;border-right:0">비고</td>
										</tr>
                    ...
                    <div class="fm">
						<table border="0" cellspacing="0" cellpadding="0">
							<xsl:choose>
								<xsl:when test="$mode='new' or $mode='edit'">
									<tr>
                                    	<td align="left "><span>신청 장비 목록</span></td>
										<td style="text-align:right">
							...
							<tr>
								<td colspan="2">
									<table id="__subtable2" class="ft-sub" header="1"  border="0" cellspacing="0" cellpadding="0">
										<tr style="height:25">
											<td class="f-lbl-sub" style="width:28px;border-top:0">No</td>
											<td class="f-lbl-sub" style="width:;border-top:0">신청 장비 목록</td>
											<td class="f-lbl-sub" style="width:50px;border-top:0">수량</td>
                                            <td class="f-lbl-sub" style="width:150px;border-top:0;border-right:0">비고</td>
										</tr>
			...
			<td>
				<xsl:choose>
					...
					<xsl:otherwise>
						...
						<xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(USEEQUIP))" />
					</xsl:otherwise>
				</xsl:choose>
			</td>
			<td>
			...
			<td style="border-right:0">
				<xsl:choose>
					...
					<xsl:otherwise>
						...
						<xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(MEMO1))" />
					</xsl:otherwise>
				</xsl:choose>
			</td>
		</tr>
    ...
    <xsl:template match="//forminfo/subtables/subtable2/row">
		<tr class="sub_table_row">
			...
			<td>
				<xsl:choose>
					<xsl:when test="$mode='new'">
						<input type="text" name="NEEDEQUIP">
							<xsl:attribute name="class">txtText</xsl:attribute>
							<xsl:attribute name="maxlength">100</xsl:attribute>
						</input>
					</xsl:when>
					...
					<xsl:otherwise>
						...
						<xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(NEEDEQUIP))" />
					</xsl:otherwise>
				</xsl:choose>
			</td>
			<td>
			...
			<td style="border-right:0">
				<xsl:choose>
					...
					<xsl:otherwise>
						...
						<xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(MEMO2))" />
					</xsl:otherwise>
				</xsl:choose>
			</td>
		</tr>
