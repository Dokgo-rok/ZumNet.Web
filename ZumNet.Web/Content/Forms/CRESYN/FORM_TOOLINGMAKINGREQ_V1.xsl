    ...
    <html>
      <head>
        ...
        <style type="text/css">
          ...
          .m .ft .f-lbl {width:10%} .m .ft .f-lbl1 {width:9%} .m .ft .f-lbl2 {width:?}
            ...
            <table class="ft" border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td style="width:15%;border-bottom:0;">
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="CUSTOMER" style="width:87%" class="txtText_u" readonly="readonly" value="{//forminfo/maintable/CUSTOMER}" />
                ...
                <td class="f-lbl1" style="border-bottom:0;">금형비부담</td>
                <td style="border-bottom:0;width:10%;">
                  <span class="f-option2">
                    <input type="checkbox" id="ckb11" name="ckbCOSTCHARGE" value="당사">
                      <xsl:if test="$mode='new' or $mode='edit'">
                        <xsl:attribute name="onclick">parent.fnCheckYN('ckbCOSTCHARGE', this, 'COSTCHARGE')</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="phxsl:isEqual(string(//forminfo/maintable/COSTCHARGE),'당사')">
                        <xsl:attribute name="checked">true</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="$mode='read' and phxsl:isDiff(string(//forminfo/maintable/COSTCHARGE),'당사')">
                        <xsl:attribute name="disabled">disabled</xsl:attribute>
                      </xsl:if>
                    </input>
                    <label for="ckb11">당사</label>
                  </span>
                  <span class="f-option2">
                    <input type="checkbox" id="ckb12" name="ckbCOSTCHARGE" value="고객">
                      <xsl:if test="$mode='new' or $mode='edit'">
                        <xsl:attribute name="onclick">parent.fnCheckYN('ckbCOSTCHARGE', this, 'COSTCHARGE')</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="phxsl:isEqual(string(//forminfo/maintable/COSTCHARGE),'고객')">
                        <xsl:attribute name="checked">true</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="$mode='read' and phxsl:isDiff(string(//forminfo/maintable/COSTCHARGE),'고객')">
                        <xsl:attribute name="disabled">disabled</xsl:attribute>
                      </xsl:if>
                    </input>
                    <label for="ckb12">고객</label>
                  </span>
                  <input type="hidden" id="__mainfield" name="COSTCHARGE">
                    <xsl:attribute name="value">
                      <xsl:value-of select="//forminfo/maintable/COSTCHARGE"></xsl:value-of>
                    </xsl:attribute>
                  </input>
                </td>
                <td class="f-lbl" style="border-bottom:0;">진행일자</td>
                <td style="width:%;border-bottom:0;">
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="FROMDATE" class="txtDate" style="width:80px" maxlength="8" onclick="parent.fnShowPopSelfCalendar(this, parent.fnCalcValid)" value="{//forminfo/maintable/FROMDATE}" />
                    </xsl:when>
                    <xsl:otherwise>
                  ...
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="TODATE" class="txtDate" style="width:80px" maxlength="8" onclick="parent.fnShowPopSelfCalendar(this, parent.fnCalcValid)" value="{//forminfo/maintable/TODATE}" />
                    </xsl:when>
                    <xsl:otherwise>
                  ...
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="DAILY" class="txtNumberic" readonly="readonly" style="width:25px" maxlength="3" value="{//forminfo/maintable/DAILY}" />
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(//forminfo/maintable/DAILY))" />
                    </xsl:otherwise>
                  </xsl:choose>
                  &nbsp;일&nbsp;)
                </td>
                <td class="f-lbl" style="border-bottom:0;">적용일자</td>
                <td style="border-right:0;border-bottom:0;width:15%;">
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="APPLYDATE" class="txtDate" style="width:80px" maxlength="8" onclick="parent.fnShowPopSelfCalendar(this, parent.fnCalcValid)" value="{//forminfo/maintable/APPLYDATE}" />
                    </xsl:when>
                    <xsl:otherwise>
                ...
                <td colspan="2">
                  <table id="__subtable1" class="ft-sub" header="1" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr>
                      ...
                      <td class="f-lbl-sub" style="border-top:0;">금형방식</td>
  ...
  <xsl:template match="//forminfo/subtables/subtable1/row">
    <tr class="sub_table_row">
      ...
      <td style="text-align:center">
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="TOOLINGTYPE" class="txtText_u" readonly="readonly" value="{TOOLINGTYPE}" style="width:90%"/>
            <button onclick="parent.fnOption('external.toolingmakingreq',300,180,200,20,'etc',this);" onfocus="this.blur()" class="btn_bg" style="height:16px;">
