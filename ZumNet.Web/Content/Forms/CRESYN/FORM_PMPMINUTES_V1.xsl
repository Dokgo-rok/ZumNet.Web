    ...
    <html>
      <head>
        ...
        <style type="text/css">
          ...
          /* 화면 넓이, 에디터 높이, 양식명크기 */
          .m {width:750px} .m .fm-editor {height:600px;border:windowtext 0pt solid}
            ...
            <table class="ft" border="0" cellspacing="0" cellpadding="0" text-align="center">
              ...
              <tr>
                ...
                <td colspan="3" style="border-bottom:0;border-right:0">
                  <span class="f-option">
                    <input type="checkbox" id="ckb11" name="ckbEVENTDEGREE" value="PMP1">
                      <xsl:if test="$mode='new' or $mode='edit'">
                        <xsl:attribute name="onclick">parent.fnCheckYN('ckbEVENTDEGREE', this, 'EVENTDEGREE')</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="phxsl:isEqual(string(//forminfo/maintable/EVENTDEGREE),'PMP1')">
                        <xsl:attribute name="checked">true</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="$mode='read' and phxsl:isDiff(string(//forminfo/maintable/EVENTDEGREE),'PMP1')">
                        <xsl:attribute name="disabled">disabled</xsl:attribute>
                      </xsl:if>
                    </input>
                    <label for="ckb11">PMP1</label>
                  </span>
                  <span class="f-option">
                    <input type="checkbox" id="ckb12" name="ckbEVENTDEGREE" value="PMP2">
                      <xsl:if test="$mode='new' or $mode='edit'">
                        <xsl:attribute name="onclick">parent.fnCheckYN('ckbEVENTDEGREE', this, 'EVENTDEGREE')</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="phxsl:isEqual(string(//forminfo/maintable/EVENTDEGREE),'PMP2')">
                        <xsl:attribute name="checked">true</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="$mode='read' and phxsl:isDiff(string(//forminfo/maintable/EVENTDEGREE),'PMP2')">
                        <xsl:attribute name="disabled">disabled</xsl:attribute>
                      </xsl:if>
                    </input>
                    <label for="ckb12">PMP2</label>
                  </span>
                  <span class="f-option">
                    <input type="checkbox" id="ckb13" name="ckbEVENTDEGREE" value="PMP3">
                      <xsl:if test="$mode='new' or $mode='edit'">
                        <xsl:attribute name="onclick">parent.fnCheckYN('ckbEVENTDEGREE', this, 'EVENTDEGREE')</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="phxsl:isEqual(string(//forminfo/maintable/EVENTDEGREE),'PMP3')">
                        <xsl:attribute name="checked">true</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="$mode='read' and phxsl:isDiff(string(//forminfo/maintable/EVENTDEGREE),'PMP3')">
                        <xsl:attribute name="disabled">disabled</xsl:attribute>
                      </xsl:if>
                    </input>
                    <label for="ckb13">PMP3</label>
                  </span>
      ...
      <td>
        ...
        <span class="f-option">
          <input type="checkbox" name="ckbCHECKINGRESULT" value="NG">
            <xsl:attribute name="id">
              ckb.<xsl:value-of select="ROWSEQ" />.CB
            </xsl:attribute>
            <xsl:if test="$mode='new' or $mode='edit'">
              <xsl:attribute name="onclick">parent.fnTableCheckYN('ckbCHECKINGRESULT', this, 'CHECKINGRESULT')</xsl:attribute>
            </xsl:if>
