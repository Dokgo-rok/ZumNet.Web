      ...
      <head>
        ...
        <style type="text/css">
          <xsl:value-of select="phxsl:baseStyle()" />
			/* 화면 넓이, 에디터 높이, 양식명크기 */
			.m {width:700px} .m .fm-editor {height:500px;min-height:500px;border:windowtext 1pt solid}
            ...
            <table class="ft" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                ...
                <td style="border-right:0">
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="MODELNAME" class="txtRead" readonly="readonly"  style="width:91%" value="{//forminfo/maintable/MODELNAME}" />
              ...
              <tr>
                <td class="f-lbl" style="">개발단계</td>
                <td style="">
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="STEP" style="width:115px">
                      ...
                      <!--<button onclick="parent.fnOption('external.devstep',140,120,130,50,'etc','STEP');" onfocus="this.blur()" class="btn_bg" style="height:16px;">
						...
						<button type="button" class="btn btn-outline-secondary btn-18" title="개발단계" onclick="_zw.formEx.optionWnd('external.devstep',140,214,-118,0,'etc','STEP');">
							<i class="fas fa-angle-down"></i>
						</button>
                    </xsl:when>
                    <xsl:otherwise>
                ...
                <td>
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="LEVEL" style="width:79px">
                        <xsl:attribute name="class">txtText_u</xsl:attribute>
                        <xsl:attribute name="readonly">readonly</xsl:attribute>
                        <xsl:attribute name="value">
                      ...
                      <!--<button onclick="parent.fnOption('external.devlevel',140,120,130,50,'etc','LEVEL');" onfocus="this.blur()" class="btn_bg" style="height:16px;">
						...
						<button type="button" class="btn btn-outline-secondary btn-18" title="LEVEL" onclick="_zw.formEx.optionWnd('external.devlevel',140,184,-100,0,'etc','LEVEL');">
							<i class="fas fa-angle-down"></i>
						</button>
                    </xsl:when>
                    <xsl:otherwise>
                  ...
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      ...
                      <!--<button onclick="parent.fnOption('external.customecode',140,120,130,50,'etc','CUSTOMER');" onfocus="this.blur()" class="btn_bg" style="height:16px;">
						...
						<button type="button" class="btn btn-outline-secondary btn-18" title="고객명" onclick="_zw.formEx.optionWnd('external.customecode',200,300,-100,0,'etc','CUSTOMER');">
							<i class="fas fa-angle-down"></i>
						</button>
                    </xsl:when>
                    <xsl:otherwise>
              ...
              <tr>
                  ...
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      ...
                      <!--<button onclick="parent.fnOption('external.centercode',200,120,130,50,'etc','LOCATION');" onfocus="this.blur()" class="btn_bg" style="height:16px;">
						...
						<button type="button" class="btn btn-outline-secondary btn-18" title="생산지" onclick="_zw.formEx.optionWnd('external.centercode',240,304,-220,0,'etc','LOCATION');">
							<i class="fas fa-angle-down"></i>
						</button>
                    </xsl:when>
                    <xsl:otherwise>
                ...
                <td style="border-right:0">
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="REQDEPT" >
                        <xsl:attribute name="class">txtText</xsl:attribute>
                        <xsl:attribute name="maxlength">100</xsl:attribute>
                        <xsl:attribute name="value">
                          <xsl:value-of select="//forminfo/maintable/REQDEPT" />
                        </xsl:attribute>
                      </input>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(//forminfo/maintable/REQDEPT))" />
                    </xsl:otherwise>
                  </xsl:choose>
                </td>
              </tr>
              <tr>
                <td class="f-lbl" style="">제품상태</td>
                <td style="" colspan="2">
                  <span style="width:70px">
                    <input type="checkbox" id="ckb11" name="ckbPRODUCT" value="완제품">
                      <xsl:if test="$mode='new' or $mode='edit'">
                        <xsl:attribute name="onclick">_zw.form.checkYN('ckbPRODUCT', this, 'PRODUCT')</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="phxsl:isEqual(string(//forminfo/maintable/PRODUCT),'완제품')">
                        <xsl:attribute name="checked">true</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="$mode='read' and phxsl:isDiff(string(//forminfo/maintable/PRODUCT),'완제품')">
                        <xsl:attribute name="disabled">disabled</xsl:attribute>
                      </xsl:if>
                    </input>
                    <label for="ckb11">완제품</label>
                  </span>
                  <span style="width:70px" >
                    <input type="checkbox" id="ckb12" name="ckbPRODUCT" value="부품">
                      <xsl:if test="$mode='new' or $mode='edit'">
                        <xsl:attribute name="onclick">_zw.form.checkYN('ckbPRODUCT', this, 'PRODUCT')</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="phxsl:isEqual(string(//forminfo/maintable/PRODUCT),'부품')">
                        <xsl:attribute name="checked">true</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="$mode='read' and phxsl:isDiff(string(//forminfo/maintable/PRODUCT),'부품')">
                        <xsl:attribute name="disabled">disabled</xsl:attribute>
                      </xsl:if>
                    </input>
                    <label for="ckb12">부품</label>
                  </span>
                  <input type="text" id="__mainfield" name="CHECKPRODUCT" style="width:96px">
                    <xsl:attribute name="class">txtText_u</xsl:attribute>
                    <xsl:attribute name="readonly">readonly</xsl:attribute>                
                    <xsl:attribute name="value">
                      <xsl:value-of select="//forminfo/maintable/CHECKPRODUCT" />
                    </xsl:attribute>
                  </input>
                  <input type="hidden" id="__mainfield" name="PRODUCT">
                    <xsl:attribute name="value">
                      <xsl:value-of select="//forminfo/maintable/PRODUCT"></xsl:value-of>
                    </xsl:attribute>
                  </input>
                </td>
                <td class="f-lbl" style="">설계담당자</td>
                <td style="border-right:0">
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="SPECCHARGER" >
                        <xsl:attribute name="class">txtText</xsl:attribute>
                        <xsl:attribute name="maxlength">100</xsl:attribute>
                        <xsl:attribute name="value">
                          <xsl:value-of select="//forminfo/maintable/SPECCHARGER" />
                        </xsl:attribute>
                      </input>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(//forminfo/maintable/SPECCHARGER))" />
                    </xsl:otherwise>
                  </xsl:choose>
                </td>
              </tr>
              <tr>
                <td class="f-lbl" style="">시료수량</td>
                <td style="text-align:center" colspan="2">
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="PRODUCOUNT" class="txtNumberic" maxlength="20" data-inputmask="number;20;0" value="{//forminfo/maintable/PRODUCOUNT}" />
                    </xsl:when>
                    <xsl:otherwise>
                ...
                <td class="f-lbl" style="">기준모델</td>
                <td style="border-right:0">
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="STANDMODEL" >
                        <xsl:attribute name="class">txtText</xsl:attribute>
                        <xsl:attribute name="maxlength">100</xsl:attribute>
                        <xsl:attribute name="value">
                          <xsl:value-of select="//forminfo/maintable/STANDMODEL" />
                        </xsl:attribute>
                      </input>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(//forminfo/maintable/STANDMODEL))" />
                    </xsl:otherwise>
                  </xsl:choose>
                </td>
              </tr>
              <tr>
                <td class="f-lbl" style="border-bottom:0">검토완료희망일</td>
                <td style="border-bottom:0;text-align:center" colspan="2" >
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="HOPEDATE" class="datepicker txtDate" maxlength="10" data-inputmask="date;yyyy-MM-dd" value="{//forminfo/maintable/HOPEDATE}" />
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(//forminfo/maintable/HOPEDATE))" />
                    </xsl:otherwise>
                  </xsl:choose>
                </td>
                ...
                <td style="border-right:0;border-bottom:0">
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      ...
                      <!--<button onclick="parent.fnOption('external.devclass',80,110,100,30,'','DEVCLASS');" onfocus="this.blur()" class="btn_bg" style="height:16px;">
                        <img alt="" class="blt01" style="margin:0 0 2px 0">
                          <xsl:attribute name="src">/<xsl:value-of select="$root"/>/EA/Images/ico_28.gif</xsl:attribute>
                        </img>
                      </button>-->
						<button type="button" class="btn btn-outline-secondary btn-18" title="개발등급" onclick="_zw.formEx.optionWnd('external.devclass',120,120,-100,0,'','DEVCLASS');">
							<i class="fas fa-angle-down"></i>
						</button>
                    </xsl:when>
                    <xsl:otherwise>
