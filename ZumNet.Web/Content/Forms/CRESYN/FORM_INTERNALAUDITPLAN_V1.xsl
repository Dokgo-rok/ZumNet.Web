        ...
        <div class="m">
          ...
          <div class="fm">
            <table class="ft" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                <td class="f-lbl" style="border-bottom:0;">
                  AuditTeam<br />Leader审核队长
                </td>
                <td style="border-bottom:0;">
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="TEAMLEADER" class="txtText" maxlength="30" value="{//forminfo/maintable/TEAMLEADER}" />
                    </xsl:when>
                    <xsl:otherwise>
                ...
                <td style="border-bottom:0;border-right:0;">
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="TEAMMEMBER" class="txtText" maxlength="50" value="{//forminfo/maintable/TEAMMEMBER}" />
                    </xsl:when>
                    <xsl:otherwise>
          ...
          <div class="fm">
            <span>1. Internal Audit Detailed Contents 内部审核内容</span>
            
            <div class="ff" />
            <div class="ff" />

            &nbsp;&nbsp;<span>1) Audit Contents 审核内容</span>
          </div>
          <div class="ff" />
            ...
            <table class="ft" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td style="border-right:0;border-bottom:0">
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <textarea id="__mainfield" name="AUDITCONTENTS" style="height:150px" class="txaText" onkeyup="parent.checkTextAreaLength(this, 3000);">
                ...
                <td colspan="2">
                  <table id="__subtable1" class="ft-sub" header="1" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr>
                      ...
                      <td class="f-lbl-sub" style="border-top:0;font-size:8pt">Audit Date<br />审核日子
                      </td>
                      <td class="f-lbl-sub" style="border-top:0;font-size:8pt" colspan="3">Audit Time<br />审核时间
                      </td>
                      <td class="f-lbl-sub" style="border-top:0;font-size:8pt">Department<br />部门/对</td>
                      <td class="f-lbl-sub" style="border-top:0;font-size:8pt">Audit Place<br />审核地方
                      </td>
                      <td class="f-lbl-sub" style="border-top:0;font-size:8pt">Auditor<br />审核员
                      </td>
                      <td class="f-lbl-sub" style="border-top:0;border-right:0;font-size:8pt">ETC<br />其他</td>
                    </tr>
          ...
          <div class="fm">
            <span>2. Significant Report 其他内容</span>
          </div>
          <div class="ff" />
            ...
            <table class="ft" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td style="border-right:0;border-bottom:0">
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <textarea id="__mainfield" name="SPECIALTY" style="height:100px" class="txaText" onkeyup="parent.checkTextAreaLength(this, 2000);">
                        <xsl:value-of select="//forminfo/maintable/SPECIALTY" />
                      </textarea>
                    </xsl:when>
                    <xsl:otherwise>
                      <div id="__mainfield" name="PROGRESSION" style="height:100px">
                        <xsl:attribute name="class">txaRead</xsl:attribute>
  ...
  <xsl:template match="//forminfo/subtables/subtable1/row">
    <tr class="sub_table_row">
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="FROMDATE" class="txtDate" maxlength="8" onclick="parent.fnShowPopSelfCalendar(this, parent.fnCalcValid)" value="{FROMDATE}" />
          </xsl:when>
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Center</xsl:attribute>
      ...
      <td style="border-right:0;">
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="FROMTIME" class="txtHHmm" maxlength="4" value="{FROMTIME}" />
          </xsl:when>
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Center</xsl:attribute>
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="TOTIME" class="txtHHmm" maxlength="4" value="{TOTIME}" />
          </xsl:when>
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Center</xsl:attribute>
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="AUDITDEPT" class="txtText" maxlength="30" value="{AUDITDEPT}" />
          </xsl:when>
          <xsl:otherwise>
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="AUDITPLACE" class="txtText" maxlength="50" value="{AUDITPLACE}" />
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(AUDITPLACE))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="AUDITOR" class="txtText" maxlength="30" value="{AUDITOR}" />
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(AUDITOR))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td style="border-right:0">
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="ETC" class="txtText" maxlength="300" value="{ETC}" />
          </xsl:when>
          <xsl:otherwise>
