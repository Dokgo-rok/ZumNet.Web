        ...
        <div class="m">
          ...
          <div class="fm">
            <table class="ft" border="0" cellspacing="0" cellpadding="0" text-align="center">
                ...
                <td >
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      ...
                      <button onclick="parent.fnOption('external.centercode',200,140,120,60,'','PRODUCECENTER');" onfocus="this.blur()" class="btn_bg" style="height:16px;">
                        <img alt="" class="blt01" style="margin:0 0 2px 0">
                          <xsl:attribute name="src">
                            /<xsl:value-of select="$root"/>/EA/Images/ico_28.gif
                          </xsl:attribute>
              ...
              <tr>
                <td class="f-lbl1" style="border-bottom:0">분석일</td>
                <td style="border-bottom:0">
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="ANALYSISDATE">
                        ...
                        <xsl:attribute name="value">
                          <xsl:value-of select="//forminfo/maintable/ANALYSISDATE" />
                        </xsl:attribute>
                      </input>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(//forminfo/maintable/ANALYSISDATE))" />
                    </xsl:otherwise>
                  </xsl:choose>
                </td>
                <td class="f-lbl" style="border-bottom:0">분석결과</td>
                <td style="border-bottom:0;border-right:0;text-align:center;">
                  <span class="f-option">
                    <input type="checkbox" id="ckb11" name="ckbANALYSISRESULT" value="합격">
                      <xsl:if test="$mode='new' or $mode='edit'">
                        <xsl:attribute name="onclick">parent.fnCheckYN('ckbANALYSISRESULT', this, 'ANALYSISRESULT')</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="phxsl:isEqual(string(//forminfo/maintable/ANALYSISRESULT),'합격')">
                        <xsl:attribute name="checked">true</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="$mode='read' and phxsl:isDiff(string(//forminfo/maintable/ANALYSISRESULT),'합격')">
                  ...
                  <span class="f-option">
                    <input type="checkbox" id="ckb12" name="ckbANALYSISRESULT" value="불합격">
                      <xsl:if test="$mode='new' or $mode='edit'">
                        <xsl:attribute name="onclick">parent.fnCheckYN('ckbANALYSISRESULT', this, 'ANALYSISRESULT')</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="phxsl:isEqual(string(//forminfo/maintable/ANALYSISRESULT),'불합격')">
                        <xsl:attribute name="checked">true</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="$mode='read' and phxsl:isDiff(string(//forminfo/maintable/ANALYSISRESULT),'불합격')">
                        <xsl:attribute name="disabled">disabled</xsl:attribute>
                      </xsl:if>
                    </input>
                    <label for="ckb12">불합격</label>
                  </span>
                  <input type="hidden" id="__mainfield" name="ANALYSISRESULT">
                    <xsl:attribute name="value">
                      <xsl:value-of select="//forminfo/maintable/ANALYSISRESULT"></xsl:value-of>
                    </xsl:attribute>
                  </input>
                </td> 
              </tr>
