            ...
            <table class="ft" border="0" cellspacing="0" cellpadding="0" style="border-top:0">
              <tr>
                <td style="border-bottom:0;border-right:0;height:100px">
                  <span style="width:100%;text-align:center">위와 같이 야간근무를 신청하오니 허락하여 주시기 바랍니다.</span>
                </td>
              </tr>
            </table>
          </div>
  ...
  <xsl:template match="//forminfo/subtables/subtable1/row">
    <tr class="sub_table_row">
      ...
      <td class="tdRead_Center">
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text_u" name="NFROMTIME" readonly="readonly"  class="txtHHmm" maxlength="4" value="22:00"  onBlur="parent.fnTimecheck(this)" />
          </xsl:when>
          <xsl:otherwise>
      ...
      <td class="tdRead_Center">
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="NTOTIME" class="txtHHmm" maxlength="4" value="{NTOTIME}" onBlur="parent.fnTimecheck(this)" />
          </xsl:when>
          <xsl:otherwise>
