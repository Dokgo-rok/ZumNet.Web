								...
								<td colspan="2">
									<table id="__subtable1" class="ft-sub" header="1"  border="0" cellspacing="0" cellpadding="0">
										...
										<tr style="height:40px">
											...
											<td class="f-lbl-sub" style="border-top:0">Factory</td>
											<td class="f-lbl-sub" style="border-top:0">수량</td>
											<td class="f-lbl-sub" style="border-top:0">DEMAND TYPE</td>
											<td class="f-lbl-sub" style="border-top:0">SO No.(FSCT 일자)</td>
											<td class="f-lbl-sub" style="border-top:0">Project No.</td>
											<td class="f-lbl-sub" style="border-top:0">고객 PO No.</td>
											<td class="f-lbl-sub" style="border-top:0">고객 보상여부</td>
											<td class="f-lbl-sub" style="border-top:0;border-right:0">취소 사유 및 대책</td>
										</tr>
	...
	<xsl:template match="//forminfo/subtables/subtable1/row">
		<tr class="sub_table_row">
			...
			<td>
				<xsl:choose>
					<xsl:when test="$mode='new' or $mode='edit'">
						<input type="text" name="MODELNAME" style="width:92%">
			...
			<td>
				<xsl:choose>
					<xsl:when test="$mode='new' or $mode='edit' ">
						<input type="text" id="__mainfield" name="REVCNT"  class="txtDollar" maxlength="10" data-inputmask="number;10;0" value="{//forminfo/subtable1/REVCNT}" />
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(//forminfo/subtable1/REVCNT))" />
					</xsl:otherwise>
				</xsl:choose>
			</td>
			<td>
				<xsl:choose>
					<xsl:when test="$mode='new' or $mode='edit' ">
						<input type="text" name="DEMANDTYPE">
							<xsl:attribute name="class">txtText</xsl:attribute>
					...
					<xsl:otherwise>
						...
						<xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(DEMANDTYPE))" />
					</xsl:otherwise>
				</xsl:choose>
			</td>
			<td style="text-align:center">
				<xsl:choose>
					<xsl:when test="$mode='new' or $mode='edit'">
						<select name="SONUM" class="form-control">
							<xsl:choose>
								<xsl:when test="phxsl:isEqual(string(SONUM),'')">
									<option value="" selected="selected">선택</option>
								</xsl:when>
								<xsl:otherwise>
									<option value="">선택</option>
							...
							<xsl:choose>
								<xsl:when test="phxsl:isEqual(string(SONUM),'S/O')">
									<option value="S/O" selected="selected">S/O</option>
								</xsl:when>
								<xsl:otherwise>
							...
							<xsl:choose>
								<xsl:when test="phxsl:isEqual(string(SONUM),'FCST')">
									<option value="FCST" selected="selected">FCST</option>
								</xsl:when>
								<xsl:otherwise>
					...
					<xsl:otherwise>
						...
						<xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(SONUM))" />
					</xsl:otherwise>
				</xsl:choose>
			</td>
			<td>
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:attribute name="class">tdRead_Right</xsl:attribute>
						<xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(PROJECTNUM))" />
					</xsl:otherwise>
				</xsl:choose>
			</td>
			<td>
				<xsl:choose>
					<xsl:when test="$mode='new' or $mode='edit' ">
						<input type="text" name="CUSTOMERPONUM" class="txtDollar" maxlength="10" data-inputmask="number;10;0" value="{CUSTOMERPONUM}" />
					</xsl:when>
					<xsl:otherwise>
						<xsl:attribute name="class">tdRead_Right</xsl:attribute>
						<xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(CUSTOMERPONUM))" />
					</xsl:otherwise>
				</xsl:choose>
			</td>
			<td style="text-align:center">
				<xsl:choose>
					<xsl:when test="$mode='new' or $mode='edit'">
						<select name="COMPENSATIONSTATUS" class="form-control">
							<xsl:choose>
								<xsl:when test="phxsl:isEqual(string(COMPENSATIONSTATUS),'')">
									<option value="" selected="selected">선택</option>
								</xsl:when>
								<xsl:otherwise>
									<option value="">선택</option>
							...
							<xsl:choose>
								<xsl:when test="phxsl:isEqual(string(CHECKKOREA),'O')">
							...
							<xsl:choose>
								<xsl:when test="phxsl:isEqual(string(CHECKKOREA),'X')">
					...
					<xsl:otherwise>
						<xsl:attribute name="class">tdRead_Center</xsl:attribute>
						<xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(COMPENSATIONSTATUS))" />
					</xsl:otherwise>
				</xsl:choose>
			</td>
			<td>
