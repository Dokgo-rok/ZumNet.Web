                ...
                <td colspan="2">
                  <table id="__subtable1" class="ft-sub" header="2"  border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr style="height:24px">
                      ...
                      <td class="f-lbl-sub" style="border-top:0" rowspan="2">변경구분</td>
                      <td class="f-lbl-sub" style="border-top:0" rowspan="2">MODEL</td>
                      <td class="f-lbl-sub" style="border-top:0" rowspan="2">ITEM</td>
                      <td class="f-lbl-sub" style="border-top:0" rowspan="2">DESCRIPTION</td>
                      <td class="f-lbl-sub" style="border-top:0" colspan="2">변경사항</td>
                      <td class="f-lbl-sub" style="border-top:0" rowspan="2">변경사유/근거</td>
                      <td class="f-lbl-sub" style="border-top:0" rowspan="2">적용시점<br />(년/월/일)</td>
                     <td class="f-lbl-sub" style="border-top:0;border-right:0" rowspan="2">재고사용관계</td>
                    </tr>
                    <tr style="height:24px">
  ...
  <xsl:template match="//forminfo/subtables/subtable1/row">
    <tr class="sub_table_row">
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="PRODUCTCENTER" style="width:75%">
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="CHANGECLASS" style="width:85%">
              <xsl:attribute name="class">txtText_u</xsl:attribute>
              <xsl:attribute name="readonly">readonly</xsl:attribute>
              <xsl:attribute name="value">
                <xsl:value-of select="CHANGECLASS" />
              </xsl:attribute>
            </input>
            <button onclick="parent.fnOption('external.filechangecode',180,200,70,122,'',this);" onfocus="this.blur()" class="btn_bg" style="height:16px;">
              <img alt="" class="blt01" style="margin:0 0 2px 0">
                <xsl:attribute name="src">
                  /<xsl:value-of select="$root"/>/EA/Images/ico_28.gif
                </xsl:attribute>
          ...
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(CHANGECLASS))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td>
      ...
      <td>
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(DESCRIPTION))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="BEFORECHANGE">
              <xsl:attribute name="class">txtText</xsl:attribute>
              <xsl:attribute name="maxlength">50</xsl:attribute>
              <xsl:attribute name="value">
                <xsl:value-of select="BEFORECHANGE" />
              </xsl:attribute>
            </input>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(BEFORECHANGE))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="AFTERCHANGE">
              <xsl:attribute name="class">txtText</xsl:attribute>
              <xsl:attribute name="maxlength">50</xsl:attribute>
              <xsl:attribute name="value">
                <xsl:value-of select="AFTERCHANGE" />
              </xsl:attribute>
            </input>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(AFTERCHANGE))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td>
      ...
      <td>
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(APPDATE))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td style="border-right:0">
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(RELATION))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
    </tr>
