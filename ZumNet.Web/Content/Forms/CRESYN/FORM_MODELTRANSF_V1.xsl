        ...
        <div class="m">
          ...
          <div class="fm">
            <table class="ft" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                <td class="f-lbl" style="border-bottom:0;">이동 사유 및<br /> 내역 요약</td>
    ...
    <tr class="sub_table_row">
      ...
      <td style="border:0;border-top:1px solid windowtext;padding:0;height:164px">
        <table class="ft-sub-sub" header="0" border="0" cellpadding="0" cellspacing="0">
            ...
            <td style="border-right:0">
              <xsl:choose>
                ...
                <xsl:otherwise>
                  ...
                  <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(FROMDATE))" />
                </xsl:otherwise>
              </xsl:choose>~
              <xsl:choose>
                <xsl:when test="$mode='new' or $mode='edit'">
                ...
                <xsl:otherwise>
                  <xsl:attribute name="class">tdRead</xsl:attribute>
                  <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(TODATE))" />
                </xsl:otherwise>
              </xsl:choose>
            </td>
          </tr>
          ...
          <tr class="subsub_table_row">
            <td class="f-lbl-sub">현 사업장</td>
            <td>
              <xsl:choose>
                <xsl:when test="$mode='new' or $mode='edit'">
                  <input type="text" name="PREPLACE">
                    <xsl:attribute name="class">txtRead</xsl:attribute>
                    <xsl:attribute name="readonly">readonly</xsl:attribute>
                    <xsl:attribute name="value">
                ...
                <xsl:otherwise>
                  <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(PREPLACE))" />
                </xsl:otherwise>
              </xsl:choose>
            
            </td>
            <td class="f-lbl-sub">판매처(사업장)</td>
            <td style="border-right:0">
              <xsl:choose>
                <xsl:when test="$mode='new' or $mode='edit'">
              ...
              <input type="hidden" name="MOVEPLACEID">
                <xsl:attribute name="value">
                  <xsl:value-of select="MOVEPLACEID" />
                </xsl:attribute>
              </input>
              <input type="hidden" name="MOVEPLACESITEID">
                <xsl:attribute name="value">
                  <xsl:value-of select="MOVEPLACESITEID" />
                </xsl:attribute>
              </input>
            </td>
          </tr>
          <tr class="subsub_table_row">
            ...
            <td class="f-lbl-sub">제작금액</td>
            <td style="border-right:0">
              <xsl:choose>
                <xsl:when test="$mode='new' or $mode='edit'">
                  <input type="text" name="MAKEMONEY">
                    <xsl:attribute name="class">txtText</xsl:attribute>                  
                    <xsl:attribute name="value">
                      <xsl:value-of select="MAKEMONEY" />
                    </xsl:attribute>
                  </input>
                </xsl:when>
                <xsl:otherwise>
                  <xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(MAKEMONEY))" />
                </xsl:otherwise>
              </xsl:choose>
            </td>
          </tr>
