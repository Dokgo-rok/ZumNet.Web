                ...
                <td colspan="2">
                  <table id="__subtable1" class="ft-sub" header="1"  border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr style="height:25">
                      ...
                      <td class="f-lbl-sub" style="border-top:0">예상금액</td>
  ...
  <xsl:template match="//forminfo/subtables/subtable1/row">
    <tr class="sub_table_row">
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="UNITCOST" class="txtDollar" maxlength="20" data-inputmask="number;16;4" value="{UNITCOST}" />
          </xsl:when>
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Right</xsl:attribute>
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="QUANTITY" class="txtVolume" maxlength="10" data-inputmask="number;10;0" value="{QUANTITY}" />
          </xsl:when>
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Right</xsl:attribute>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(QUANTITY))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
