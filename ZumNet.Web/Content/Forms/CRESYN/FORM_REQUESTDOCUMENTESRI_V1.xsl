...
<xsl:stylesheet  version="1.0"
		...
		<html>
			<head>
				...
				<style type="text/css">
					...
					/* 폰트 */
					.fh h1 {font-family:굴림체;font-weight:bold;font-size:20.0pt;letter-spacing:3pt;}
			...
			<body>
				<div class="m">
					...
					<div class="fm">
						<table class="ft" border="0" cellspacing="0" cellpadding="0">
							...
							<tr>
								...
								<td>
									<input type="text" name="SUBMISSION" id="__mainfield">
										<xsl:choose>
											...
											<xsl:when test="$mode='edit'">
												...
												<xsl:attribute name="maxlength">50</xsl:attribute>
												<xsl:attribute name="value">
													<xsl:value-of select="//forminfo/maintable/SUBMISSION" />
												</xsl:attribute>
											</xsl:when>
											<xsl:otherwise>
												<xsl:attribute name="class">txtRead</xsl:attribute>
								...
								<td style="border-right:0">
									<input type="text" name="DUEDATE" id="__mainfield">
										<xsl:choose>
											<xsl:when test="$mode='new'">
							...
							<tr>
								<td class="f-lbl" style="border-bottom:0">용도</td>
								<td colspan="3" style="border-bottom:0">
									<input type="text" name="USAGE" id="__mainfield">
										<xsl:choose>
											<xsl:when test="$mode='new'">
												<xsl:attribute name="class">txtText</xsl:attribute>
												<xsl:attribute name="maxlength">50</xsl:attribute>
					...
					<div class="fm">
						<span>※ 제출처가 다수일 경우"000 외"로 작성하시고 특이사항에 상세 작성하시기 바랍니다.</span>
					</div>
							...
							<tr>
								<td>
									<table id="__subtable1" class="ft-sub" header="1"  border="0" cellspacing="0" cellpadding="0">
										<tr style="height:25">
											<td colspan="2" class="f-lbl-sub" style="border-top:0;width:90%">서류종류</td>
											<td class="f-lbl-sub" style="border-top:0;border-right:0">매수</td>
										</tr>
          ...
          <div class="fm">
            <table class="ft" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td class="f-lbl" style="border-right:0">특이사항</td>
              </tr>
              <tr>
                <td style="height:200px;vertical-align:top;border-right:0;">
                  <xsl:choose>
                    <xsl:when test="$mode='read'">
                      <div name="REFERENCE" id="__mainfield">
                        <xsl:attribute name="class">txaRead</xsl:attribute>
                        <xsl:value-of disable-output-escaping="yes" select="phxsl:replaceTextArea(string(//forminfo/maintable/REFERENCE))" />
                      </div>
                    </xsl:when>
                    <xsl:otherwise>
                      <textarea name="REFERENCE" id="__mainfield">
          ...
          -->
					...
					<div class="fm">
						<table class="ft" border="0" cellspacing="0" cellpadding="0">
							<tr>
								<td style="border-bottom:0" class="f-lbl">특이사항</td>
								<td style="border-bottom:0;border-right:0;height:200px;vertical-align:top">
									<xsl:choose>
										<xsl:when test="$mode='read'">
											<div name="REFERENCE" id="__mainfield">
												<xsl:attribute name="class">txaRead</xsl:attribute>
										...
										<xsl:otherwise>
											<textarea name="REFERENCE" id="__mainfield" style="height:200px">
												<xsl:attribute name="class">txaText</xsl:attribute>
												<xsl:attribute name="onkeyup">parent.checkTextAreaLength(this, 1000)</xsl:attribute>
												<xsl:if test="$mode='edit'">
													<xsl:value-of select="//forminfo/maintable/REFERENCE" />
					...
					<!--
					<div class="fm">
						<span>상기와 같이 근태계를 제출하오니 재가하여 주시기 바랍니다.</span>
					</div>
          -->
	...
	<xsl:template match="//forminfo/subtables/subtable1/row">
		<tr class="sub_table_row" style="border-bottom:0">
			<td style="width:5%">
