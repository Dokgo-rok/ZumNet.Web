      ...
      <head>
        ...
        <style type="text/css">
          <xsl:value-of select="phxsl:baseStyle()" />
			/* 화면 넓이, 에디터 높이, 양식명크기 */
			.m {width:1100px} .m .fm-editor {height:400px;min-height:400px;border:windowtext 1pt solid}
                ...
                <td colspan="2">
                  <table id="__subtable1" class="ft-sub" header="1" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr style="height:24px">
                      ...
                      <td class="f-lbl-sub" style="border-top:0;border-right:0">입금확인</td>
                    </tr>
     ...
     <td>
       <xsl:choose>
        ...
        <span class="f-option">
           <input type="checkbox" name="ckbPARTITION" value="유상">
             ...
             <xsl:if test="$mode='new' or $mode='edit'">
               <xsl:attribute name="onclick">_zw.form.checkTableYN('ckbPARTITION', this, 'PARTITION')</xsl:attribute>
             </xsl:if>
             <xsl:if test="phxsl:isEqual(string(PARTITION),'유상')">
               <xsl:attribute name="checked">true</xsl:attribute>
             </xsl:if>
             <xsl:if test="$mode='read' and phxsl:isDiff(string(PARTITION),'유상')">
               <xsl:attribute name="disabled">disabled</xsl:attribute>
             </xsl:if>
           </input>
			<label class="ml-1">
            <xsl:attribute name="for">ckb.<xsl:value-of select="ROWSEQ" />.A</xsl:attribute>유상
          </label>
         </span>
         <span class="f-option">
           <input type="checkbox" name="ckbPARTITION" value="무상">
             ...
             <xsl:if test="$mode='read' and phxsl:isDiff(string(PARTITION),'무상')">
               <xsl:attribute name="disabled">disabled</xsl:attribute>
             </xsl:if>
           </input>
			 <label class="ml-1">
             <xsl:attribute name="for">ckb.<xsl:value-of select="ROWSEQ" />.B</xsl:attribute>무상
           </label>
         </span>
      ...
      <td style="border-right:0">
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(CONFIRMATION))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
    </tr>
