            ...
            <table class="ft" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td class="f-lbl">성명
                  <xsl:if test="$mode='new' or $mode='edit'">
                    <!--<button onclick="parent.fnOrgmap('ur','N');" onfocus="this.blur()" class="btn_bg" style="height:16px;">
					  ...
					  <button type="button" class="btn btn-outline-secondary btn-18" data-toggle="tooltip" data-placement="bottom" title="대상자" onclick="_zw.fn.org('user','n', 'APPLICANT');">
                ...
                <td style="width:;border-right:0">
                  <xsl:choose>
                    <xsl:when test="$mode='new'">
                      <input type="text" id="__mainfield" name="APPLICANTINDATE">
                        <xsl:attribute name="class">txtRead</xsl:attribute>
                        <xsl:attribute name="readonly">readonly</xsl:attribute>
                        <xsl:attribute name="value">
                    ...
                    <xsl:when test="$mode='edit'">
                      <input type="text" id="__mainfield" name="APPLICANTINDATE">
                        ...
                        <xsl:attribute name="readonly">readonly</xsl:attribute>
                        <xsl:attribute name="value">
                          <xsl:value-of select="//forminfo/maintable/APPLICANTINDATE" />
                        </xsl:attribute>
                      </input>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(//forminfo/maintable/APPLICANTINDATE))" />
                    </xsl:otherwise>
                  </xsl:choose>
                </td>
              </tr>
              <tr>
                <td class="f-lbl" style="border-bottom:0">소속</td>
            ...
            <table border="0" cellspacing="0" cellpadding="0">
              <xsl:choose>
                <xsl:when test="$mode='new' or $mode='edit'">
                  <tr>
                    <td>
                      <span>2. 주요경력 및 과거 포상기록 ( 훈장, 포장, 표창 )</span>
                    </td>
					  <td class="fm-button">
                ...
                <xsl:otherwise>
                  <tr>
                    <td>
                      <span>2. 주요겅력 및 과거 포상기록 ( 훈장, 포장, 표창 )</span>
                    </td>
                  </tr>
                </xsl:otherwise>
              </xsl:choose>
                ...
                <td colspan="2">
                  <table id="__subtable1" class="ft-sub" header="1" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr style="height:24px">
                      ...
                      <td class="f-lbl-sub" style="border-top:0">포상날짜</td>
                      <td class="f-lbl-sub" style="border-top:0;border-right:0">주요경력내용</td>
                    </tr>
            ...
            <table class="ft" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td style="border-bottom:0;border-right:0">
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <textarea id="__mainfield" name="REMARK" style="height:100px" class="txaText bootstrap-maxlength" maxlength="2000">
  ...
  <xsl:template match="//forminfo/subtables/subtable1/row">
    <tr class="sub_table_row">
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="PRIZEDATE" class="datepicker txtDate" maxlength="10" data-inputmask="date;yyyy-MM-dd" value="{PRIZEDATE}" />
          </xsl:when>
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Center</xsl:attribute>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(PRIZEDATE))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td style="border-right:0">
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="PRIZECONTENT" class="txtText" maxlength="50" value="{PRIZECONTENT}" />
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(PRIZECONTENT))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
    </tr>
