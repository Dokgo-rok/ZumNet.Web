                ...
                <td colspan="2">
                  <table id="__subtable1" class="ft-sub" header="1" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr style="height:24px">
                      ...
                      <td class="f-lbl-sub" style="border-top:0">추진일</td>
                      <td class="f-lbl-sub" style="border-top:0">추진항목</td>
                      <td class="f-lbl-sub" style="border-top:0;">추진비</td>
    ...
    <tr class="sub_table_row">
      ...
      <td>
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(WORKTITLE))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="WORKMONEY">
              <xsl:attribute name="class">txtDollar</xsl:attribute>
              <xsl:attribute name="maxlength">50</xsl:attribute>
              <xsl:attribute name="value">
                <xsl:value-of select="WORKMONEY" />
              </xsl:attribute>
            </input>
          </xsl:when>
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Right</xsl:attribute>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(WORKMONEY))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>   
      <td style="border-right:0">
