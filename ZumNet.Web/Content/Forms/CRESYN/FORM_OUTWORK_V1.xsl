            ...
            <table class="ft" border="0" cellspacing="0" cellpadding="0" style="border-top:0">
              <tr>
                <td style="border-bottom:0;border-right:0;height:100px">
                  <span style="width:100%;text-align:center">위와 같이 외부근무를 신청하오니 허락하여 주시기 바랍니다.</span>
                </td>
              </tr>
            </table>
          </div>
  ...
  <xsl:template match="//forminfo/subtables/subtable1/row">
    <tr class="sub_table_row">
      ...
      <td class="tdRead_Center">
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="OFROMTIME" class="txtHHmm" maxlength="4" value="{OFROMTIME}"  onBlur="parent.fnTimecheck(this)" />
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(OFROMTIME))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td class="tdRead_Center">
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="OTOTIME" class="txtHHmm" maxlength="4" value="{OTOTIME}" onBlur="parent.fnTimecheck(this)" />
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(OTOTIME))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
	  <td class="tdRead_Center">
		<xsl:choose>
		    <xsl:when test="$mode='new' or $mode='edit'">
			    <input type="text" name="TARGETPLACE" class="txtText" maxlength="100"  value="{TARGETPLACE}" />
		    </xsl:when>
		    <xsl:otherwise>
			    <xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(TARGETPLACE))" />
		    </xsl:otherwise>
		</xsl:choose>
	  </td>
      <td style="border-right:0">
