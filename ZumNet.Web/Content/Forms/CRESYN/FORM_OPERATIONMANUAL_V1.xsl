        ...
        <div class="m">
          ...
          <div class="fm">
            <table class="ft" border="0" cellspacing="0" cellpadding="0" text-align="center">
              ...
              <tr>
                <td class="f-lbl" style="height:40px">품명<br />Product name</td>
                <td>
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                ...
                <td class="f-lbl">모델명<br />Model name</td>
                <td  style="border-right:0">
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
              ...
              <tr>
                <td class="f-lbl" style="height:40px">고객명<br />Customer name</td>
                <td>
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                ...
                <td class="f-lbl">발행단계<br />Develop step</td>
                <td style="border-right:0">
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
              ...
              <tr>
                <td class="f-lbl1" style="border-bottom:0;height:40px">생산지<br />Production site</td>
                <td colspan="3" style="border-bottom:0;border-right:0">
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="PRODUCECENTER" style="width:37%">
                        <xsl:attribute name="class">txtText_u</xsl:attribute>
                        <xsl:attribute name="readonly">readonly</xsl:attribute>
                        <xsl:attribute name="value">
                      ...
                      <!--<button onclick="parent.fnOption('external.centercode',200,140,120,-10,'','PRODUCECENTER');" onfocus="this.blur()" class="btn_bg" style="height:16px;">
						...
						<button type="button" class="btn btn-outline-secondary btn-18" title="생산지" onclick="_zw.formEx.optionWnd('external.centercode',240,274,-200,0,'','PRODUCECENTER');">
							<i class="fas fa-angle-down"></i>
						</button>
                    </xsl:when>
                    <xsl:otherwise>
           ...
           <table border="0" cellspacing="0" cellpadding="0">
              <xsl:choose>
                <xsl:when test="$mode='new' or $mode='edit'">
                  <tr>
                    <td>
                      <span>모델적용 작업지도서 리스트 Work guidebook list</span>
                    </td>
                    <td class="fm-button">
                ...
                <td colspan="2">
                  <table id="__subtable1" class="ft-sub" header="1" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr style="height: 50px">
                      ...
                      <td class="f-lbl-sub" style="border-top:0">표준번호<br />Standard number</td>
                      <td class="f-lbl-sub" style="border-top:0">작업지도서명<br />Workguidebook Name</td>
                      <td class="f-lbl-sub" style="border-top:0">적용 공정명<br />Process Name</td>
                      <td class="f-lbl-sub" style="border-top:0">페이지수<br />page numbers</td>
                        <td class="f-lbl-sub" style="border-top:0">제/개정일<br />Reversion date</td>
                        <td class="f-lbl-sub" style="border-top:0;border-right:0">비고<br />Remarks</td>
                    </tr>
    ...
    <tr class="sub_table_row">
      ...
      <td>
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(STDNO))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td  style="border-bottom:0">
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(BOOKNAME))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td  style="border-bottom:0">
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(PROCESSNAME))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
		<td class="tdRead_Center">
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="PAGENO" class="txtNumberic" maxlength="20" data-inputmask="number;20;0" value="{PAGENO}" />
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(PAGENO))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
		<td class="tdRead_Center">
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" id="__mainfield" name="REVISIONDATE" class="datepicker txtDate" maxlength="10" data-inputmask="date;yyyy-MM-dd" value="{REVISIONDATE}" />
          </xsl:when>
          <xsl:otherwise>
