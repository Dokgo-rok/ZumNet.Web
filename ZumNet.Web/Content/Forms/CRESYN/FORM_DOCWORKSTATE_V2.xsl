        ...
        <div class="m">
          ...
          <div class="fm">
            <table class="ft" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                <td colspan="4" class="ft-ttl" style="text-align:center;vertical-align:middel;font-size:40px;height:140px">
                  Certificate Of Employment<br/>
                  (재 직 증 명 서)
                </td>
              </tr>
              <tr>
                <td class="f-lbl2" style="text-align:center;font-size:16px !important; vertical-align:middle;" rowspan="3"  >
              ...
              <tr>
                ...
                <td colspan="2" style="font-size:16px;padding-left:5px">
                  <xsl:choose>
                    <xsl:when test="$mode='new'">
                     <span> <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(//forminfo/maintable/ENGNM))" />
                      (<xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(//forminfo/maintable/APPLICANT))" />) </span>
              ...
              <tr>
                ...
                <td colspan="2" style="text-align:left;font-size:16px;padding-left:5px">
                  <xsl:choose>
                    <xsl:when test="$mode='new'">
                      <span> <xsl:value-of select="phxsl:convertDate(string(//forminfo/maintable/BIRTHDATE),'en')" /> 
                      (<xsl:value-of select="phxsl:convertDate(string(//forminfo/maintable/BIRTHDATE),'ko')" />)</span>
                  ...
                  <xsl:choose>
                    <xsl:when test="$mode='new'">
                      <span><xsl:value-of select="string(//forminfo/maintable/ENGDEPTNM)" />
                        (<xsl:value-of select="string(//forminfo/maintable/APPLICANTDEPT)" />)</span>
                  ...
                  <xsl:choose>
                    <xsl:when test="$mode='new'">
                     <span> <xsl:value-of select="string(//forminfo/maintable/ENGGRADENM)" />
                      (<xsl:value-of select="string(//forminfo/maintable/APPLICANTGRADE)" />)</span>
                  ...
                  <xsl:choose>
                    <xsl:when test="$mode='new'">
                      <span style="text-align:left">
                        ...
                        <xsl:value-of select="phxsl:convertDate(string(//docinfo/createdate),'en')" /> <br/>
                        (<xsl:value-of select="phxsl:convertDate(string(//forminfo/maintable/INDATE),'ko')" /> &nbsp;부터 &nbsp;&nbsp;
                        <xsl:value-of select="phxsl:convertDate(string(//docinfo/createdate),'ko')" />&nbsp;현재)
                      </span>
                    ...
                    <xsl:when test="$mode='edit'">
                      <span style="text-align:left"> <xsl:value-of select="phxsl:convertDate(string(//forminfo/maintable/INDATE),'en')" /> ~
                      <xsl:value-of select="phxsl:convertDate(string(//forminfo/maintable/REQUESTDATE),'en')" /> <br/>
                      (<xsl:value-of select="phxsl:convertDate(string(//forminfo/maintable/INDATE),'ko')" /> &nbsp;부터 &nbsp;&nbsp;
                      <xsl:value-of select="phxsl:convertDate(string(//forminfo/maintable/REQUESTDATE),'ko')" />&nbsp;현재)
                    </span>
                      <input type="hidden" id="__mainfield" name="INDATE" value="{//forminfo/maintable/INDATE}"/>
                      <input type="hidden" id="__mainfield" name="REQUESTDATE" value="{//forminfo/maintable/REQUESTDATE}"/>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of disable-output-escaping="yes" select="phxsl:convertDate(string(//forminfo/maintable/INDATE),'en')" /> ~
                      <xsl:value-of disable-output-escaping="yes" select="phxsl:convertDate(string(//forminfo/maintable/REQUESTDATE),'en')" /> <br/>
                      (<xsl:value-of disable-output-escaping="yes" select="phxsl:convertDate(string(//forminfo/maintable/INDATE),'ko')" />&nbsp;부터 &nbsp;&nbsp;
                      <xsl:value-of disable-output-escaping="yes" select="phxsl:convertDate(string(//forminfo/maintable/REQUESTDATE),'ko')" />&nbsp;현재)
                  </xsl:otherwise>
                  </xsl:choose>
                </td>
              </tr>
              ...
              <tr>
                ...
                <td style ="border-top:0; border-bottom:0;border-left:0; padding-left:10px;padding-top:20px">
                  <xsl:if test="//bizinfo/@docstatus='700'">
