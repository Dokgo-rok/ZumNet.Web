            ...
            <table border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td style="width:470px">
                  <xsl:value-of disable-output-escaping="yes" select="phxsl:mappingSignRcvPart($root, //processinfo/signline/lines, 'receive', '__si_Receive', '6', '주관부서')"/>
                </td>
              </tr>
            </table>
          </div>
                ...
                <td >
                  <table id="__subtable1" class="ft-sub" header="1"  border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr style="height:25">
                      ...
                      <td class="f-lbl-sub" style="border-top:0">사용자</td>
                      <td class="f-lbl-sub" style="border-top:0">일자</td>
                      <td class="f-lbl-sub" style="border-top:0">시간</td>
                      <td class="f-lbl-sub" style="border-top:0">출발장소</td>
                      <td class="f-lbl-sub" style="border-top:0">경유지</td>
                      <td class="f-lbl-sub" style="border-top:0">도착장소</td>
                      <td class="f-lbl-sub" style="border-top:0">금액</td>
                      <td class="f-lbl-sub" style="border-top:0;border-right:0">사유</td>
                    </tr>
                    <xsl:apply-templates select="//forminfo/subtables/subtable1/row"/>
                    <tr>
                      <td colspan="7" class="f-lbl-sub">TOTAL</td>
                      <td style="border-bottom:0">
                        <xsl:choose>
                          <xsl:when test="$mode='new' or $mode='edit'">
    ...
    <tr class="sub_table_row">
      ...
      <td>
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Center</xsl:attribute>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(DRIVER))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td>
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Center</xsl:attribute>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(DRIVEDATE))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="DRIVETIME" class="txtHHmm text-center" maxlength="5" data-inputmask="time;HH:MM" value="{DRIVEDATE}" />
          </xsl:when>
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Center</xsl:attribute>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(DRIVETIME))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="DRIVESTART">
              <xsl:attribute name="class">txtText</xsl:attribute>
              <xsl:attribute name="maxlength">200</xsl:attribute>
              <xsl:attribute name="style">text-align:center</xsl:attribute>
              <xsl:attribute name="value">
                <xsl:value-of select="DRIVESTART" />
              </xsl:attribute>
            </input>
          </xsl:when>
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Center</xsl:attribute>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(DRIVESTART))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="DRIVECENTER">
              <xsl:attribute name="class">txtText</xsl:attribute>
              <xsl:attribute name="maxlength">200</xsl:attribute>
              <xsl:attribute name="style">text-align:center</xsl:attribute>
              <xsl:attribute name="value">
                <xsl:value-of select="DRIVECENTER" />
              </xsl:attribute>
            </input>
          </xsl:when>
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Center</xsl:attribute>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(DRIVECENTER))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="DRIVEEND">
              <xsl:attribute name="class">txtText</xsl:attribute>
              <xsl:attribute name="maxlength">200</xsl:attribute>
              <xsl:attribute name="style">text-align:center</xsl:attribute>
              <xsl:attribute name="value">
                <xsl:value-of select="DRIVEEND" />
              </xsl:attribute>
            </input>
          </xsl:when>
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Center</xsl:attribute>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(DRIVEEND))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td style="">
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="SUM" class="txtDollar" maxlength="20" data-inputmask="number;16;4" value="{SUM}" />
          </xsl:when>
          <xsl:otherwise>
            <xsl:attribute name="class">tdRead_Center</xsl:attribute>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(SUM))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td style="border-right:0">
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="DRIVEREASON">
              <xsl:attribute name="class">txtText</xsl:attribute>
              <xsl:attribute name="maxlength">200</xsl:attribute>
              <xsl:attribute name="style">text-align:center</xsl:attribute>
              <xsl:attribute name="value">
                <xsl:value-of select="DRIVEREASON" />
              </xsl:attribute>
            </input>
          </xsl:when>
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(DRIVEREASON))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
    </tr>
