...
<xsl:stylesheet  version="1.0"
    ...
    <html>
      <head>
        ...
        <style type="text/css">
          ...
          .m .ft .f-option {width:15%} .m .ft .f-option1 {width:24%;} .m .ft .f-option2 {width:9%;} .m .ft .f-option3 {width:20%;}
        ...
        <div class="m">
            ...
            <table border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td style="width:450px">
                  <xsl:value-of disable-output-escaping="yes" select="phxsl:mappingSignSerialPart($root, //processinfo/signline/lines/line[(@bizrole='부품수배' or @bizrole='샘플제작'  or @bizrole='제품검토' or @bizrole='최종검토' or @bizrole='재고확인' or @bizrole='승인결과') and @partid!='' and @step!='0'], '__si_Receive', '6', '검토부서')"/>
                </td>
              </tr>
            </table>
          </div>
          ...
          <div class="fm">
            <table class="ft" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                ...
                <td class="f-lbl" style="border-bottom:0">ECN NO.</td>
          ...
          <div class="fm">
            <table class="ft" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                <td class="f-lbl" style="height:30px">변경사유</td>
                <td colspan="3" style="text-align:center;border-right:0">
                  <span class="f-option">
                  ...
                  (<xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="ETCREASON" style="width:200px">
                        <xsl:attribute name="class">txtRead</xsl:attribute>
                        <xsl:attribute name="maxlength">50</xsl:attribute>
                        <xsl:attribute name="readonly">readonly</xsl:attribute>
                        <xsl:attribute name="value">
          ...
          <div class="fm">
            <table id="__si_Form"  class="ft" border="0" cellspacing="0" cellpadding="0" style="border-bottom:0">              
              ...
              <xsl:apply-templates select="//processinfo/signline/lines/line[ (@bizrole='부품수배' or @bizrole='수배의뢰' or @bizrole='검토의뢰' or @bizrole='부품검토' or @bizrole='샘플제작' or @bizrole='제품검토'  or @bizrole='최종검토'  or @bizrole='재고파악'   or @bizrole='재고확인'  or @bizrole='승인결과'   or @bizrole='변통발행') and @actrole='_approver' and @partid!='']">
  ...
  <xsl:template match="//processinfo/signline/lines/line">
    <tr>
      <td style="text-align:center">
        <xsl:choose>
          <xsl:when test="phxsl:isEqual(string(@bizrole),'수배의뢰')">
            부품수배의뢰
          </xsl:when>
          <xsl:when test="phxsl:isEqual(string(@bizrole),'부품수배')">
            부품수배
          </xsl:when>
          <xsl:when test="phxsl:isEqual(string(@bizrole),'검토의뢰')">
            부품검토의뢰
          </xsl:when>
          <xsl:when test="phxsl:isEqual(string(@bizrole),'부품검토')">
            부품검토
          </xsl:when>
          <xsl:when test="phxsl:isEqual(string(@bizrole),'샘플제작')">
            샘플제작
          </xsl:when>
          <xsl:when test="phxsl:isEqual(string(@bizrole),'제품검토')">
            샘플검토
          </xsl:when>
          <xsl:when test="phxsl:isEqual(string(@bizrole),'최종검토')">
            최종검토
          </xsl:when>
          <xsl:when test="phxsl:isEqual(string(@bizrole),'재고파악')">
            재고확인요청
          </xsl:when>
          <xsl:when test="phxsl:isEqual(string(@bizrole),'재고확인')">
            재고통보
          </xsl:when>
          <xsl:when test="phxsl:isEqual(string(@bizrole),'승인결과')">
            고객승인
          </xsl:when>
          <xsl:when test="phxsl:isEqual(string(@bizrole),'변통발행')">
            변통발행
          </xsl:when>
        </xsl:choose>
      </td>
