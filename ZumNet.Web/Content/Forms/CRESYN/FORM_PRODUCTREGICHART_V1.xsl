      ...
      <head>
        ...
        <style type="text/css">
          <xsl:value-of select="phxsl:baseStyle()" />
			/* 화면 넓이, 에디터 높이, 양식명크기 */
			.m {width:770px} .m .fm-editor {height:500px;min-height:500px;border:windowtext 1pt solid}
          ...
          <div class="fm">
            <table class="ft" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                ...
                <td>
                  <xsl:choose>
                    ...
                    <xsl:otherwise>
                      <xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(//forminfo/maintable/EXPECTATIONDAY))" />
                    </xsl:otherwise>
                  </xsl:choose>
                </td>

                <td class="f-lbl1">기획수량</td>
                <td style="border-right:0">
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="COUNT" class="txtCurrency" maxlength="20" data-inputmask="number;20;0" value="{//forminfo/maintable/COUNT}" />
                    </xsl:when>
                    <xsl:otherwise>
              ...
              <tr>
                    ...
                    <xsl:choose>
                      <xsl:when test="$mode='new' or $mode='edit'">
                        ...
                        <!--<button onclick="parent.fnOption('external.centercode',200,140,150,-10,'','PRODUCECENTER');" onfocus="this.blur()" class="btn_bg" style="height:16px;">
						  ...
						  <button type="button" class="btn btn-outline-secondary btn-18" title="생산지" onclick="_zw.formEx.optionWnd('external.centercode',200,140,150,-10,'','PRODUCECENTER');">
							  <i class="fas fa-angle-down"></i>
						  </button>
                      </xsl:when>
                      <xsl:otherwise>
                ...
                <td class="f-lbl" style="border-bottom:0">모델명(UNIT)</td>
                <td style="border-right:0;border-bottom:0">
                  <xsl:choose>
                    ...
                    <xsl:otherwise>
                      <xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(//forminfo/maintable/UNIT))" />
                    </xsl:otherwise>
                  </xsl:choose>
                </td>
