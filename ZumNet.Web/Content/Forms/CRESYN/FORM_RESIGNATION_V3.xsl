...
<xsl:stylesheet  version="1.0"
    ...
    <html>
      <head>
        ...
        <style type="text/css">
          <xsl:value-of select="phxsl:baseStyle()" />
			/* 화면 넓이, 에디터 높이, 양식명크기 */
			.m {width:720px} .m .fm-editor {height:350px;min-height:350px;border:windowtext 0pt solid}
      ...
      <body>
        <div class="m">
            ...
            <table class="ft" border="0" cellspacing="0" cellpadding="0">
                ...
                <td class="f-lbl">성명
                  <xsl:if test="$mode='new' or $mode='edit'">
                    <!--<button onclick="parent.fnOrgmap('ur','N','RSNNM');" onfocus="this.blur()" class="btn_bg" style="height:16px;">
					  ...
					  <button type="button" class="btn btn-outline-secondary btn-18" data-toggle="tooltip" data-placement="bottom" title="Contacts" onclick="_zw.fn.org('user','n','RSNNM');">
              ...
              <tr>
                ...
                <td>
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="RSNCORP" class="txtText_u" readonly="readonly" value="{//creatorinfo/belong}" style="width:48%" />&nbsp;.&nbsp;
                      <input type="text" id="__mainfield" name="RSNDEPT" class="txtText_u" readonly="readonly" value="{//creatorinfo/department}" style="width:45%" />
                    </xsl:when>
                    <xsl:otherwise>
                ...
                <td style="border-right:0">
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="RSNWORK" class="txtText" maxlength="50" value="{//forminfo/maintable/RSNWORK}" onblur="_zw.formEx.blur(this)" />
                    </xsl:when>
                    <xsl:otherwise>
              ...
              <tr>
                ...
                <td>
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="RSNINDATE" style="width:80px" class="datepicker txtDate" maxlength="10" data-inputmask="date;yyyy-MM-dd" value="{//creatorinfo/indate}" />&nbsp;~&nbsp;
                      <input type="text" id="__mainfield" name="RSNOUTDATE" style="width:80px" class="datepicker txtDate" maxlength="10" data-inputmask="date;yyyy-MM-dd" value="{//forminfo/maintable/RSNOUTDATE}" />
                    </xsl:when>
                    <xsl:otherwise>
          ...
          <xsl:if test="$mode='new' or $mode='edit' or ($bizrole='application' and $actrole='__r' and $partid!='') or count(//processinfo/signline/lines/line[(@bizrole='normal' or @bizrole='application') and @partid=$uid])>0">
            <div class="fm text-center" id="panReason">
              <!--<xsl:if test="not ($mode='new' or $mode='edit' or $bizrole='normal' or $bizrole='application')">-->
              <span class="f-option mb-1" style="width:100%;font-size:13px">상기 본인은 다음의 사유로 인하여 사직코져 하오니 재가하여 주시기 바립니다.</span>
              <table class="ft" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td style="border-right:0;border-bottom:0">
                    <xsl:choose>
                      <xsl:when test="$mode='new' or $mode='edit'">
                        <textarea id="__mainfield" name="REASON" style="height:100px" class="txaText bootstrap-maxlength" maxlength="2000">
                          <xsl:value-of select="//forminfo/maintable/REASON" />
                        </textarea>
                      </xsl:when>
                      <xsl:otherwise>
                        <div class="txaRead" style="height:100px;padding:2px">
				...
				<h4 style="margin-top:8px; margin-bottom: 32px">크레신 주식회사 귀하</h4>
            </div>
          </xsl:if>

          <span class="l_pos" style="width:100%; text-align:center">
            <h4 style="margin-top:30px">본인이 작성하였음을 확인합니다.</h4>
          ...
          <xsl:if test="$mode='new' or $mode='edit' or ($bizrole='application' and $actrole='__r' and $partid!='') or count(//processinfo/signline/lines/line[(@bizrole='normal' or @bizrole='application') and @partid=$uid])>0">
            <div class="fm text-center" id="panStatement">
              <h3 style="margin-bottom:4px">영업비밀 보호 준수 서약서</h3>
              <h5>(퇴사자용)</h5>
              <p style="text-align:left">
                ...
                (이하 '본인'이라 한다)는 크레신주식회사(이하 '회사'라 한다)를 퇴사하면서 아래사항을 이행하기로 하고, 만약 이행하지 않을 때에는 민사▪형사상의 책임을 지기로 하고 본 서약서를 회사에게 제출한다.
              </p>
              <p>- 아&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;래 -</p>
              <ol>
                <li>본인은 퇴직일로부터 2년이내 동업종 경쟁업체에 취업하지 아니한다.</li>
                <li>
                  <table style="width:630px" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      <td style="width:30px">가.</td>
                      <td>본인은 회사에 고용되어 재직한 기간중에 지득한 영업비밀 및 보안사항을 제 3자에게 누설하지 아니한다.</td>
                    </tr>
                    <tr>
                      <td>나.</td>
                      <td>본인은 퇴직일로부터 2년간 회사에 고용되어 재직한 기간 중에 지득한 영업비밀을 이용하기 위한 목적으로 창업하거나, 경쟁사에 전직,동업,고문,자문,기타 협력의 지위를 가질 수 없다.</td>
                    </tr>
                    <tr>
                      <td>다.</td>
                      <td>
                        본인은 퇴직 시, 회사에서 업무상 목적으로 개설한 SNS, SNS 대화방, 카카오톡 대화방 등 SNS(Social Network Services/Sites)에서 즉시 탈퇴할 것이며, 이 과정에서 지득한 회사와 관련된 정보 일체를 삭제하고 유출시키지 않을 것을 서약한다.
                      </td>
                    </tr>
                  </table>
                </li>
                <li>본인은 퇴직 시, 본인이 관리하고 있던 도표, 설계도, 명세서, 메모, 보고서, 자기 테이브, 디스크, 파일, 기타 기록매체 등 영업비밀과 관련된 사항이 들어 있는 일체의 자료를 회사에 반납하고, 이에 관해 어떠한 형태의 사본도 개인적으로 보유하지 않는다.</li>
                <li>본인은 회사의 기타 영업비밀 유지를 위한 법적▪도덕적 의무를 성실히 이행한다.</li>
                <li>만약 본인이 본 서약서를 위반하는 등의 행위로 회사에게 손해를 끼쳤을 경우에 본인은 회사가 입은 일체의 손해를 배상할 책임을 진다.</li>
              </ol>
              ...
              <ul class="ft-ul" style="margin-left:300px">
                <li>
              ...
              <h4 style="margin-top:32px; margin-bottom: 32px">크레신 주식회사 대표이사 귀하</h4> 
          ...
          <xsl:if test="$mode='new' or $mode='edit' or ($bizrole='application' and $actrole='__r' and $partid!='') or count(//processinfo/signline/lines/line[(@bizrole='normal' or @bizrole='application') and @partid=$uid])>0">          
            <div class="fm text-center" id="panStatement">
              <h3 style="">퇴직 직원용 - [개인정보의 이용에 관한 동의서]</h3>
              <ol>
                <li>
                  본인은 본인의 퇴직 후에도 다음과 같이 크레신주식회사가 <u>본인 및 본인 가족의</u> 개인 정보·민감<br />
                  정보·고유식별정보를 이용하는 것에 동의합니다.
                  <table  class="ft" style="width:660px"  border="0" cellspacing="0" cellpadding="0">
                    <tr>
                  ...
                  <table style="width: 660px; margin-bottom: 8px">
                    <tr>
					  ...
					  <tr>
                        ...
                        <span class="l_pos" style="text-align:center">
                          ...
                          <label class="ml-1" for="ckb11">동의함</label>
                          ...
                          <label class="ml-1" for="ckb12">동의하지 않음</label>)
                        </span>
                      </td>
                    </tr>
                  </table>
                    ...
                    <tr>
                      <td style="text-align:right">                        
                        <span class="l_pos2" style="text-align:right">
                          ...
                          <xsl:if test="$mode='new' or $mode='edit'">
                            <xsl:attribute name="onclick">_zw.form.checkYN('ckbAGREE2', this, 'AGREE2')</xsl:attribute>
                          </xsl:if>
				  ...
				  <tr>
					  <td style="text-align:center">
                        <span class="l_pos" style="text-align:center">
                          ...
                          <label class="ml-1" for="ckb21">동의함</label>
                          <input type="checkbox" id="ckb22" name="ckbAGREE2" value="미동의">                            
							...
							<label class="ml-1" for="ckb22">동의하지 않음</label>)
                        </span>
                      </td>
                    </tr>
                  </table>
                    ...
                    <tr>
                      <td style="text-align:right">                        
                        <span class="l_pos2" style="text-align:right">
                          ...
                          <xsl:if test="$mode='new' or $mode='edit'">
                            <xsl:attribute name="onclick">_zw.form.checkYN('ckbAGREE3', this, 'AGREE3')</xsl:attribute>
                          </xsl:if>
					  ...
					  <td style="text-align:center">
                        <span class="l_pos" style="text-align:center">
                          고유식별정보의 수집·이용에(<input type="checkbox" id="ckb31" name="ckbAGREE3" value="동의함">                            
							...
							<label class="ml-1" for="ckb31">동의함</label>
                          <input type="checkbox" id="ckb32" name="ckbAGREE3" value="미동의">                            
							...
							<label class="ml-1" for="ckb32">동의하지 않음</label>)
                        </span>
                      </td>
                    </tr>
                  </table>
                      ...
                      <td style="text-align: right">
                        <span class="l_pos2" style="text-align:right">
                          개인정보의 수집·이용에 동의합니다<input type="checkbox" id="ckb41" name="ckbAGREE4" value="동의함">
                            <xsl:if test="$mode='new' or $mode='edit'">
                              <xsl:attribute name="onclick">_zw.form.checkYN('ckbAGREE4', this, 'AGREE4')</xsl:attribute>
                            </xsl:if>
					  ...
					  <td style="text-align:center">
                        <span class="l_pos" style="text-align:center">
                          개인정보의 수집·이용에(<input type="checkbox" id="ckb41" name="ckbAGREE4" value="동의함">                            
							...
							<label class="ml-1" for="ckb41">동의함</label>
                          ...
                          <label class="ml-1" for="ckb42">동의하지 않음</label>)
                        </span>
                      </td>
                    </tr>
                  </table>
                      ...
                      <td style="text-align:right;width:650px">
                        <span class="l_pos2" style="text-align:right">
                          개인정보의 수집·이용에 동의합니다<input type="checkbox" id="ckb51" name="ckbAGREE5" value="동의함">
                            <xsl:if test="$mode='new' or $mode='edit'">
                              <xsl:attribute name="onclick">_zw.form.checkYN('ckbAGREE5', this, 'AGREE5')</xsl:attribute>
                            </xsl:if>
					  ...
					  <td style="text-align:center">
                        <span class="l_pos" style="text-align:center">
                          개인정보의 수집·이용에(<input type="checkbox" id="ckb51" name="ckbAGREE5" value="동의함">                            
							...
							<label class="ml-1" for="ckb51">동의함</label>
                          <input type="checkbox" id="ckb52" name="ckbAGREE5" value="미동의">                            
							...
							<label class="ml-1" for="ckb52">동의하지 않음</label>)
                        </span>
                      </td>
                    </tr>
                  </table>
              ...
              <ul class="ft-ul" style="margin-left:350px">                
				...
				<h4 style="margin-top:32px; margin-bottom: 32px">크레신 주식회사 귀하</h4>
          ...
          <xsl:if test="$mode='new' or $mode='edit' or ($bizrole='application' and $actrole='__r' and $partid!='') or count(//processinfo/signline/lines/line[(@bizrole='normal' or @bizrole='application') and @partid=$uid])>0">
            <div class="fm text-center" id="panHandover">
              <h3>업무 인계인수 확인서</h3>
              <table class="ft" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td class="f-lbl">인계자</td>
                  <td class="f-lbl1" style="width:64px">소속</td>
                ...
                <tr>
                  ...
                  <td colspan="4" style="width:600px;border:0;padding:0">
                ...
                <tr>
                  ...
                  <td class="f-lbl1" style="border-top:0">
                    소속
                    <xsl:if test="$mode='new' or $mode='edit'">
                      <!--<button onclick="parent.fnOrgmap('ur','N','OSVNM');" onfocus="this.blur()" class="btn_bg" style="height:16px;">
                        <img alt="" class="blt01" style="margin:0 0 2px 0" src="/{$root}/EA/Images/ico_28.gif" />
                      </button>-->
					<button type="button" class="btn btn-outline-secondary btn-18" data-toggle="tooltip" data-placement="bottom" title="Contacts" onclick="_zw.fn.org('user','n','OSVNM');">
		              <i class="fas fa-angle-down"></i>
	                </button>
                    </xsl:if>
                  </td>
                  ...
                  <td class="f-lbl1" style="border-top:0">성명</td>
          ...
          <div class="fm" id="panCheckout">
            <h3 style="text-align:center;margin-top: 32px">개인 지급품 및 정산 확인</h3>
            <span class="f-option" style="margin-bottom: 4px; font-size:13px">* 퇴사시 정리사항</span>
            ...
            <span class="f-option" style="margin-top: 4px; font-size:13px">※ 이 문서는 퇴사시 정산되어야 할 내역이 관련부서로 자동 통지되기위한 목적입니다.</span>
          </div>
  ...
  <xsl:template match="//forminfo/subtables/subtable1/row">
    <tr class="sub_table_row">
      ...
      <td class="f-lbl-sub" style="border-top:0; border-left:0; ">
        소속
        <xsl:if test="$mode='new' or $mode='edit'">
          <!--<button onclick="parent.fnOrgmap('ur','N',this);" onfocus="this.blur()" class="btn_bg" style="height:16px;">
            <img alt="" class="blt01" style="margin:0 0 2px 0" src="/{$root}/EA/Images/ico_28.gif" />
          </button>-->
			<button type="button" class="btn btn-outline-secondary btn-18" data-toggle="tooltip" data-placement="bottom" title="Contacts" onclick="_zw.fn.org('user','n', this);">
				<i class="fas fa-angle-down"></i>
			</button>
        </xsl:if>
      </td>
