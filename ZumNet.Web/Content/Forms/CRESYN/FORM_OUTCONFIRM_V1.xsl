        ...
        <div class="m">
          ...
          <div class="fm">
            <table class="ft" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                <td class="f-lbl" style="border-bottom:0">계산방법</td>
                <td colspan="4" style="border-bottom:0;border-right:0;height:100px">                  

                1. 급여 공제 계산식_1일 기준 : (기본급/209 * 8) * 결근일수<br /><br />
                2. 급여 공제 계산식_시간 기준 : (기본급/209) * 결근시간
        ...
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <select name="VACCLASSDN" style="width:70px">
              ...
              <xsl:choose>
                <xsl:when test="phxsl:isEqual(string(VACCLASSDN),'결근(무급)')">
                  <option value="결근(무급)" selected="selected">결근(무급)</option>
                </xsl:when>
                <xsl:otherwise>
