            ...
            <table border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td style="width:100px">                  
                  <xsl:value-of disable-output-escaping="yes" select="phxsl:mappingSignSerialPart($root, //processinfo/signline/lines/line[@bizrole='confirm' and @partid!='' and @step!='0'], '__si_Confirm', '1', '주관부서')"/>
                </td>
              </tr>
            </table>
          </div>
            ...
            <table class="ft" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                ...
                <td colspan="4" style="border-right:0">
                  <span class="f-option">
                    <input type="checkbox" id="ckb11" name="ckbOUTCLASS" value="직출">
                      <xsl:if test="$mode='new' or $mode='edit'">
                        <xsl:attribute name="onclick">parent.fnCheckYN('ckbOUTCLASS', this, 'OUTCLASS')</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="phxsl:isEqual(string(//forminfo/maintable/OUTCLASS),'직출')">
                        <xsl:attribute name="checked">true</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="$mode='read' and phxsl:isDiff(string(//forminfo/maintable/OUTCLASS),'직출')">
                        <xsl:attribute name="disabled">disabled</xsl:attribute>
                      </xsl:if>
                    </input>
                    <label for="ckb11">직출</label>
                  </span>
                  <span class="f-option">
                    <input type="checkbox" id="ckb12" name="ckbOUTCLASS" value="직퇴">
                      <xsl:if test="$mode='new' or $mode='edit'">
                        <xsl:attribute name="onclick">parent.fnCheckYN('ckbOUTCLASS', this, 'OUTCLASS')</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="phxsl:isEqual(string(//forminfo/maintable/OUTCLASS),'직퇴')">
                        <xsl:attribute name="checked">true</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="$mode='read' and phxsl:isDiff(string(//forminfo/maintable/OUTCLASS),'직퇴')">
                        <xsl:attribute name="disabled">disabled</xsl:attribute>
                      </xsl:if>
                    </input>
                    <label for="ckb12">직퇴</label>
                  </span>
                  <span class="f-option">
                    <input type="checkbox" id="ckb13" name="ckbOUTCLASS" value="직출및직퇴">
                      <xsl:if test="$mode='new' or $mode='edit'">
                        <xsl:attribute name="onclick">parent.fnCheckYN('ckbOUTCLASS', this, 'OUTCLASS')</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="phxsl:isEqual(string(//forminfo/maintable/OUTCLASS),'직출및직퇴')">
                        <xsl:attribute name="checked">true</xsl:attribute>
                      </xsl:if>
                      <xsl:if test="$mode='read' and phxsl:isDiff(string(//forminfo/maintable/OUTCLASS),'직출및직퇴')">
                        <xsl:attribute name="disabled">disabled</xsl:attribute>
                      </xsl:if>
                    </input>
                    <label for="ckb13">직출및직퇴</label>
                  </span>
                  <input type="hidden" id="__mainfield" name="OUTCLASS">
                    <xsl:attribute name="value">
                      <xsl:value-of select="//forminfo/maintable/OUTCLASS"></xsl:value-of>
                    </xsl:attribute>
                  </input>
                </td>
              </tr>
              ...
              <tr>
                ...
                <td colspan="4" style="border-right:0">
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <textarea id="__mainfield" name="OUTPLACE" style="height:60px">
                        <xsl:attribute name="class">txaText</xsl:attribute>
                        <xsl:attribute name="onkeyup">parent.checkTextAreaLength(this, 1000)</xsl:attribute>
                        <xsl:if test="$mode='edit'">
                          <xsl:value-of select="//forminfo/maintable/OUTPLACE" />
                        </xsl:if>
                      </textarea>
                    </xsl:when>
                    <xsl:otherwise>
                      <div id="__mainfield" name="OUTPLACE" style="height:60px">
                        <xsl:attribute name="class">txaRead</xsl:attribute>
                        <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(//forminfo/maintable/OUTPLACE))" />
                      </div>
                    </xsl:otherwise>
                  </xsl:choose>
                </td>
              ...
              <tr>
                ...
                <td colspan="4" style="border-right:0">
                  ...
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="FROMTIME">
                        ...
                        <xsl:attribute name="value">
                          <xsl:value-of select="//forminfo/maintable/FROMTIME" />
                        </xsl:attribute>
                      </input>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(//forminfo/maintable/FROMTIME))" />
                    </xsl:otherwise>
                  </xsl:choose>
                  &nbsp;&nbsp;~&nbsp;&nbsp;
                  <xsl:choose>
                  ...
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="TOTIME">
                        <xsl:attribute name="class">txtHHmm</xsl:attribute>
                        <xsl:attribute name="style">width:60px</xsl:attribute>
                        <xsl:attribute name="maxlength">4</xsl:attribute>
                        <xsl:attribute name="value">
                          <xsl:value-of select="//forminfo/maintable/TOTIME" />
                        </xsl:attribute>
                      </input>
                    </xsl:when>
                    <xsl:otherwise>
                      <xsl:value-of disable-output-escaping="yes" select="phxsl:isEmpty(string(//forminfo/maintable/TOTIME))" />
                    </xsl:otherwise>
                  </xsl:choose>
                  &nbsp;&nbsp;&nbsp;(
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <input type="text" id="__mainfield" name="PERIOD">
              ...
              <tr>
                <td colspan="5" style="border-bottom:0;border-right:0;height:100px">
                  <span style="width:100%;text-align:center">위와 같이 신청하오니 허락하여 주시기 바랍니다.</span>
                </td>
              </tr>
            </table>
          </div>
