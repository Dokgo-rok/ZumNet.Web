    ...
    <html>
      ...
      <body>
        <div class="m">
          ...
          <div class="fb">
            <table border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td style="width:236px">
                  <xsl:value-of disable-output-escaping="yes" select="phxsl:mappingSignSerialPart($root, //processinfo/signline/lines/line[@bizrole='agree' and @partid!='' and @step!='0'], '__si_Agree', '3', '합의권자', 'N')"/>
                </td>
                <td style="width:72px">
            ...
            <table class="ft" border="0" cellspacing="0" cellpadding="0">
              <tr>
                <td style="border-right:0;border-bottom:0;">
                  <xsl:choose>
                    <xsl:when test="$mode='new' or $mode='edit'">
                      <textarea id="__mainfield" name="CHANGEREASON" style="height:60px">
                        <xsl:attribute name="class">txaText</xsl:attribute>
                        <xsl:attribute name="onkeyup">parent.checkTextAreaLength(this, 2000)</xsl:attribute>
                        <xsl:value-of select="//forminfo/maintable/CHANGEREASON" />
                      </textarea>
                    </xsl:when>
                    <xsl:otherwise>
                      <div id="__mainfield" name="CHANGEREASON" style="height:60px">
                        <xsl:attribute name="class">txaRead</xsl:attribute>
                        <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(//forminfo/maintable/CHANGEREASON))" />
                      </div>
                    </xsl:otherwise>
                  </xsl:choose>
                </td>
                ...
                <td colspan="2">
                  <table id="__subtable1" class="ft-sub" header="1"  border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr>
                      ...
                      <td class="f-lbl-sub" rowspan="2" style="border-top:0">모델명</td>
                      <td class="f-lbl-sub" rowspan="2" style="border-top:0">고객명</td>
                      <td class="f-lbl-sub" colspan="2" style="border-top:0">생산지</td>
                      <td class="f-lbl-sub" rowspan="2" style="border-top:0">적용일</td>            
                      <td class="f-lbl-sub" rowspan="2" style="border-top:0;border-right:0">비고</td>
                    </tr>
                    <tr>
                      <td class="f-lbl-sub">변경전</td>
                      <td class="f-lbl-sub">변경후</td>
                    </tr>
          ...
          <div class="fm">
            <font color="red">
              <span>- 생산지변경수관물 요청 및 발송 LIST</span>
            </font>
          </div>
  ...
  <xsl:template match="//forminfo/subtables/subtable1/row">
    <tr class="sub_table_row">
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="CUSTOMER" style="width:80%">
              <xsl:attribute name="class">txtText_u</xsl:attribute>
              <xsl:attribute name="readonly">readonly</xsl:attribute>
              <xsl:attribute name="value">
      ...
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="BEFORECENTER" style="width:75%">
              <xsl:attribute name="class">txtText_u</xsl:attribute>
              <xsl:attribute name="readonly">readonly</xsl:attribute>
              <xsl:attribute name="value">
                <xsl:value-of select="BEFORECENTER" />
              </xsl:attribute>
            </input>
          ...
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(BEFORECENTER))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td>
        <xsl:choose>
          <xsl:when test="$mode='new' or $mode='edit'">
            <input type="text" name="AFTERCENTER" style="width:75%">
              <xsl:attribute name="class">txtText_u</xsl:attribute>
              <xsl:attribute name="readonly">readonly</xsl:attribute>
              <xsl:attribute name="value">
                <xsl:value-of select="AFTERCENTER" />
              </xsl:attribute>
            </input>
            <button onclick="parent.fnOption('external.centercode',240,120,100,0,'',this);" onfocus="this.blur()" class="btn_bg" style="height:16px;">
              <img alt="" class="blt01" style="margin:0 0 2px 0">
          ...
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(AFTERCENTER))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td style="text-align:center">
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:value-of disable-output-escaping="yes" select="phxsl:encodeHtml(string(MAKEDATE))" />
          </xsl:otherwise>
        </xsl:choose>
      </td>
      <td style="border-right:0">
